...
let chips = [1000, 1000, 1000, 1000]; // Chip counts for each player
...
let potHasBeenAwarded = false; // Track if pot has been awarded
let lastAwardedPotTransactionId = null; // Track last pot award
let currentAwardTransactionId = null; // Track current pot award
...
const playerHasActed = [false, false, false, false];  // Track players who have acted
...
// Position Tracking
// ------------------------------
let dealerIndex = 3; // Player 4 is dealer
let smallBlindIndex = 2; // Player 1 is SB
let bigBlindIndex = 1; // Player 2 is BB
let underTheGunIndex = 0; // Player 1 is UTG

// Track if user loss has been recorded for the current hand
let userLossRecorded = false;
...
// Utility to get username from URL
function getUsernameFromURL() {
  const params = new URLSearchParams(window.location.search);
  return params.get('username') || 'You';
}
...
function getSettingsStorageKey() {
    const userId = getUserIdFromURL() || 'guest';
...
function getSetting(key) {
    ...
    return settings[key] !== undefined ? settings[key] : true; // Default to true if not set
}

/**
 * Refresh settings and update UI accordingly
 * This function can be called from the main app when settings are changed
 */
function refreshSettings() {
    console.log('[SETTINGS] Refreshing settings in poker game');
    
    // Refresh hand strength display
    ...
    // Refresh tooltips by re-setting up game controls
    if (typeof setupGameControls === 'function') {
        setupGameControls();
    }
}

// Make the function available globally so it can be called from the main app
window.refreshPokerSettings = refreshSettings;
  ...
  try {
    ...
    updatedSlider.addEventListener('input', function() {
      try {
        ...
        if (currentBet > 0) {
          updatedConfirmButton.textContent = `Raise to $${currentBet + parseInt(this.value)}`;
        } else {
    ...
    function updateSliderMax() {
      try {
        ...
        if (currentBet > 0) {
          ...
          updatedConfirmButton.textContent = `Raise to $${currentBet + parseInt(updatedSlider.value)}`;
        } else {
...
function startBettingRound() {
  console.log('[DEBUG] roundStage at start of startBettingRound:', roundStage);
  if (roundStage > 3) {
    console.warn('[BUG] startBettingRound called with roundStage > 3, aborting.');
    return;
  }
  ...
  try {
    ...
    for (let i = 0; i < 4; i++) {
      // Don't reset action flag for all-in players - they can't act in new rounds
      if (!playerStates[i].allIn) {
    ...
    // Only handle blinds in the first round (pre-flop)
    if (roundStage === 0) {
      // Handle small blind automatically
      if (!playerStates[smallBlindIndex].folded) {
        const smallBlindAmount = 25;
        if (chips[smallBlindIndex] >= smallBlindAmount) {
          chips[smallBlindIndex] -= smallBlindAmount;
          currentBets[smallBlindIndex] = smallBlindAmount;
          playerHasActed[smallBlindIndex] = true;
          updateInfo(smallBlindIndex, 'smallblind', smallBlindAmount);
          console.log(`[GAME] Small blind (Player ${smallBlindIndex + 1}) posted $${smallBlindAmount}`);
        } else {
          // Handle all-in situation for small blind
          const remainingChips = chips[smallBlindIndex];
          chips[smallBlindIndex] = 0;
          currentBets[smallBlindIndex] = remainingChips;
          playerHasActed[smallBlindIndex] = true;
          addAllInIndicator(smallBlindIndex);
          updateInfo(smallBlindIndex, 'allin', remainingChips);
          console.log(`[GAME] Small blind (Player ${smallBlindIndex + 1}) is all-in with $${remainingChips}`);
        }
      }

      // Handle big blind automatically
      if (!playerStates[bigBlindIndex].folded) {
        const bigBlindAmount = 50;
        if (chips[bigBlindIndex] >= bigBlindAmount) {
          chips[bigBlindIndex] -= bigBlindAmount;
          currentBets[bigBlindIndex] = bigBlindAmount;
          playerHasActed[bigBlindIndex] = true;
          currentBet = bigBlindAmount; // Set the current bet to the big blind amount
          updateInfo(bigBlindIndex, 'bigblind', bigBlindAmount);
          console.log(`[GAME] Big blind (Player ${bigBlindIndex + 1}) posted $${bigBlindAmount}`);
        } else {
          // Handle all-in situation for big blind
          const remainingChips = chips[bigBlindIndex];
          chips[bigBlindIndex] = 0;
          currentBets[bigBlindIndex] = remainingChips;
          playerHasActed[bigBlindIndex] = true;
          currentBet = remainingChips; // Set the current bet to the all-in amount
          addAllInIndicator(bigBlindIndex);
          updateInfo(bigBlindIndex, 'allin', remainingChips);
          console.log(`[GAME] Big blind (Player ${bigBlindIndex + 1}) is all-in with $${remainingChips}`);
        }
      }

      // Update UI to reflect blind postings
      updateChipsAndBets();
    }

    // Set current player to UTG position
    currentPlayer = underTheGunIndex;
    
    // If the UTG player has folded or is all-in and has acted, find the first active player starting from UTG
    if (playerStates[currentPlayer].folded || (playerStates[currentPlayer].allIn && playerHasActed[currentPlayer])) {
      console.log(`[GAME] UTG player (${currentPlayer + 1}) has folded or is all-in and acted, finding first active player`);
    ...
    // Update UI to indicate whose turn
    ...
    console.log(`[GAME] Started betting round with player ${currentPlayer + 1} (UTG)`);
...
function showdown() {
    ...
    try {
        ...
        // Add all current bets to the pot before showdown
        for (let i = 0; i < 4; i++) {
            pot += currentBets[i];
            currentBets[i] = 0;
        }
        ...
        // Award main pot to the best hand
        const mainPotWinner = playerScores[0];
        console.log(`[GAME] Player ${mainPotWinner.player + 1} wins main pot with ${mainPotWinner.handName}`);
        
        // Move pot chips to the winner visually
        moveChipsFromPotToPlayer(mainPotWinner.player);
        
        // Delay the pot award to allow for chip animation
        setTimeout(() => {
            awardPotToPlayer(mainPotWinner.player, mainPotWinner.handName);
        }, 1000);

    } catch (error) {
        console.error("[ERROR] Showdown failed:", error);
        updateInfo("Error in showdown. Starting new game in 3 seconds...");
        setTimeout(() => {
            newGame();
        }, 3000);
...
function updateChipsAndBets() {
  try {
    ...
    // Update player info display when chips/bets change
    displayPlayerInfo();
  } catch (error) {
  ...
  try {
    ...
    if (typeof playerIndexOrMessage === 'string' && action === null) {
      ...
      if (typeof playerIndex !== 'number' || playerIndex < 0 || playerIndex > 3) {
        ...
        const playerName = playerIndex === 3 ? getUsernameFromURL() : `Player ${playerIndex + 1}`;
...
function nextRound() {
  console.log('[DEBUG] roundStage at start of nextRound:', roundStage);
  // Reset all-in state for all players before starting the next round
  cleanupAllAllInStates();
  console.log("\n=== NEXT ROUND STARTING ===");
  ...
  // Reset player action flags for new betting round
  // Note: All-in players will be marked as acted again when they can't act
  for (let i = 0; i < 4; i++) {
    if (!playerStates[i].allIn) {
      playerHasActed[i] = false;
    }
  ...
  // Update player info display after community cards are dealt
  displayPlayerInfo();
...
function handleFold(playerIndex) {
  try {
    // Prevent folding if player is all-in
    if (playerStates[playerIndex].allIn) {
        console.warn(`[WARNING] Player ${playerIndex + 1} is all-in and cannot fold.`);
        return false;
    }
    ...
    if (playerIndex === 3) {
      // Increment loss for user folding
      // Note: incrementLosses() is now handled by addRecentActivity() through updateProfileStats()
      if (!userLossRecorded) {
      ...
      // RECOVERY PATH TRACKING - Track fold discipline
      if (typeof trackFoldDiscipline === 'function') {
        const currentBetAmount = currentBet; // The amount the user would have had to call
        trackFoldDiscipline(currentBetAmount);
      }
      
      if (typeof checkAchievements === 'function') {
        checkAchievements(); // Check for fold achievements
      }
...
function goToNextPlayer(currentIdx) {
  try {
    ...
    if (currentPlayer === 3 && !playerStates[3].folded && !playerStates[3].allIn) {
      disableActionButtons(false);
      ...
      // Update player info display when it's human player's turn
      displayPlayerInfo();
    } else if (!playerStates[currentPlayer].folded && !playerStates[currentPlayer].allIn) {
      ...
      setTimeout(() => {
        try {
          ...
          if (bettingRoundActive && currentPlayer === currentPlayer && !playerStates[currentPlayer].folded && !playerStates[currentPlayer].allIn) {
            aiTakeTurn(currentPlayer);
          } else {
    ...
    } else if (playerStates[currentPlayer].allIn) {
      // All-in player can't act - mark them as acted and move to next player
      console.log(`[GAME] Player ${currentPlayer + 1} is all-in and can't act`);
      playerHasActed[currentPlayer] = true;
      disableActionButtons(true);
...
function findPreviousActivePlayer(startIndex) {
  try {
    ...
    do {
      idx = (idx + 1) % 4; // Go forward (clockwise)
      loopCounter++;
...
function handleAllIn(playerIndex, isRaise = false) {
  try {
    ...
    // Check if player is already folded
    if (playerStates[playerIndex].folded) {
      console.warn(`[WARNING] Player ${playerIndex + 1} is folded, cannot go all-in`);
      return 0;
    }
    
    // Check if player is already all-in
    if (chips[playerIndex] === 0) {
      console.warn(`[WARNING] Player ${playerIndex + 1} is already all-in`);
      return 0;
    }
    ...
    // Add dramatic pause for all-in announcement
    updateInfo(playerIndex, 'allin', allInAmount);
    
    // Place the bet (actual chip movement happens here)
    const amountBet = placeBet(playerIndex, allInAmount);
    
    // Enhanced visual feedback with staggered animations
    setTimeout(() => {
      // Animate chips moving to pot with enhanced visual effects
      moveChipsToPot(playerIndex, amountBet);
      
      // Add visual indication that player is all-in
      addAllInIndicator(playerIndex);
      
      // Add dramatic effect to the player element
      try {
        const playerElement = document.getElementById(`player-${playerIndex + 1}`);
        if (playerElement) {
          // Add temporary highlight effect (box shadow only, no scale)
          playerElement.style.boxShadow = '0 0 20px rgba(255, 87, 34, 0.8)';
        ...
        console.warn(`[WARNING] Failed to add all-in visual effects: ${uiError}`);
      }
    }, 300);
    ...
    if (currentBets[playerIndex] > currentBet) {
      ...
      // Update UI to reflect the new bet
      setTimeout(() => {
        updateChipsAndBets();
      }, 500);
    }
    
    // Mark player as all-in in game state
    playerStates[playerIndex].allIn = true;
    
    // Mark all-in player as having acted for this betting round
    playerHasActed[playerIndex] = true;
    
    // RECOVERY PATH TRACKING - Track all-in for user
    if (playerIndex === 3 && typeof trackAllIn === 'function') {
      trackAllIn();
    }
...
function addAllInIndicator(playerIndex) {
  try {
    // Validate player index
    if (playerIndex < 0 || playerIndex > 3) {
      console.error(`[ERROR] Invalid player index in addAllInIndicator: ${playerIndex}`);
      return;
    }
    
    const playerElement = document.getElementById(`player-${playerIndex + 1}`);
    if (!playerElement) {
      console.error(`[ERROR] Player element not found for index ${playerIndex + 1}`);
      return;
    }
    ...
    // Remove any existing all-in indicator first
    const existingIndicator = playerElement.querySelector('.all-in-indicator');
    if (existingIndicator) {
      existingIndicator.remove();
    }
    
    // Create an enhanced all-in indicator
    const indicator = document.createElement('div');
    indicator.className = 'all-in-indicator';
    indicator.innerHTML = `
      <span class="all-in-text">ALL IN</span>
      <span class="all-in-amount">$${chips[playerIndex] + currentBets[playerIndex]}</span>
    `;
    ...
    // Add text styling
    const textSpan = indicator.querySelector('.all-in-text');
    const amountSpan = indicator.querySelector('.all-in-amount');
    ...
    setTimeout(() => {
      ...
      // Add pulsing effect
      indicator.style.animation = 'allInPulse 2s ease-in-out infinite';
    }, 100);
    
    // Add pulsing animation to CSS if not already present
    if (!document.querySelector('#all-in-pulse-animation')) {
      const style = document.createElement('style');
      style.id = 'all-in-pulse-animation';
      style.textContent = `
    ...
    // Add subtle glow effect to the player element
    playerElement.style.filter = 'drop-shadow(0 0 10px rgba(255, 87, 34, 0.3))';
    
    console.log(`[UI] Added enhanced all-in indicator for Player ${playerIndex + 1}`);
    ...
    try {
      ...
      if (playerElement) {
        ...
        fallbackIndicator.className = 'all-in-indicator';
        fallbackIndicator.textContent = 'ALL IN';
        fallbackIndicator.style.cssText = `
          ...
          opacity: 1;
        `;
        playerElement.appendChild(fallbackIndicator);
      }
    } catch (fallbackError) {
      console.error(`[ERROR] Even fallback all-in indicator failed:`, fallbackError);
    }
  }
}
...
function updateActionButtons() {
  try {
    ...
    // Check if it's the human player's turn and they haven't folded and aren't all-in
    const yourTurn = currentPlayer === 3 && !playerStates[3].folded && !playerStates[3].allIn;
...
function shouldRunOutCards() {
  try {
    ...
    // Get active (non-folded) players with enhanced state tracking
    const activePlayers = [];
    for (let i = 0; i < 4; i++) {
      if (!playerStates[i].folded) {
        activePlayers.push({
          ...
          chips: chips[i],
          bet: currentBets[i],
          allIn: playerStates[i].allIn || chips[i] === 0,
          hasActed: playerHasActed[i]
        });
      }
    }
    ...
    if (activePlayers.length <= 1) {
      console.log(`[GAME] Only ${activePlayers.length} active player(s) - normal showdown`);
    ...
    console.log(`[GAME] All-in players: ${allInPlayers.length}, Non-all-in players: ${nonAllInPlayers.length}`);
    
    // Skip if no all-in players
    if (allInPlayers.length === 0) {
      console.log("[GAME] No all-in players detected");
      return false;
    }
    ...
    console.log(`[GAME] All players acted: ${allHaveActed}, Bets matched: ${betsMatched}`);
    
    // All-in showdown conditions:
    // 1. All active players are all-in, OR
    // 2. All but one player is all-in AND all bets have been matched AND all players have acted
    const shouldRun = (allInPlayers.length === activePlayers.length) || 
                      (nonAllInPlayers.length === 1 && betsMatched && allHaveActed);
    
    // Additional safety check: ensure we're not in the middle of a betting round
    if (shouldRun && bettingRoundActive) {
      console.log("[GAME] All-in showdown condition met, but betting round is still active");
      
      // Double-check that all non-all-in players have actually acted
      const nonAllInHaveActed = nonAllInPlayers.every(p => p.hasActed);
      if (!nonAllInHaveActed) {
        console.log("[GAME] Not all non-all-in players have acted yet");
        return false;
      }
    }
    
    console.log(`[GAME] Run out remaining cards? ${shouldRun} (${allInPlayers.length} all-in, ${nonAllInPlayers.length} with chips, bets matched: ${betsMatched}, all acted: ${allHaveActed})`);
...
function runOutRemainingCards() {
  try {
    ...
    if (window.pendingAITurn) {
      clearTimeout(window.pendingAITurn);
      window.pendingAITurn = null;
    }
    
    // Collect all bets to the pot with enhanced validation
    let totalCollected = 0;
    for (let i = 0; i < 4; i++) {
      if (currentBets[i] > 0) {
        totalCollected += currentBets[i];
        pot += currentBets[i];
        currentBets[i] = 0;
      }
    }
    
    console.log(`[GAME] Collected $${totalCollected} from outstanding bets`);
    ...
    // Enhanced UI feedback with dramatic announcement
    updateInfo("🎰 ALL-IN SHOWDOWN! 🎰 Running out remaining community cards...");
    
    // Show all player cards who are all-in with enhanced visual feedback
    setTimeout(() => {
    ...
    console.log(`[GAME] Current stage: ${roundStage}, Cards to add: ${cardsToAdd}`);
    
    // If we need to add cards, do so with enhanced animations
    if (cardsToAdd > 0) {
      ...
      if (roundStage === 0) {
        setTimeout(() => {
          try {
            updateInfo('🎴 Dealing the FLOP...');
            
            // Deal the flop (3 cards) with validation
            for (let i = 0; i < 3; i++) {
              if (deck.length > 0) {
                communityCards.push(deck.pop());
              } else {
                console.error("[ERROR] Deck is empty when trying to deal flop");
                break;
              }
            }
            
            currentStage = 1;
            showCards(false, true); // Show all-in player cards
            
            console.log(`[GAME] Flop dealt: ${communityCards.slice(-3).join(', ')}`);
          } catch (flopError) {
            console.error("[ERROR] Failed to deal flop:", flopError);
          }
        ...
        setTimeout(() => {
          try {
            ...
            if (deck.length > 0) {
              ...
              console.log(`[GAME] Turn dealt: ${communityCards[communityCards.length - 1]}`);
            } else {
              console.error("[ERROR] Deck is empty when trying to deal turn");
            }
          } catch (turnError) {
            console.error("[ERROR] Failed to deal turn:", turnError);
          }
        }, 1200 * delayMultiplier++);
      }
        ...
        setTimeout(() => {
          try {
            ...
            if (deck.length > 0) {
              ...
              console.log(`[GAME] River dealt: ${communityCards[communityCards.length - 1]}`);
            } else {
              console.error("[ERROR] Deck is empty when trying to deal river");
            }
          } catch (riverError) {
            console.error("[ERROR] Failed to deal river:", riverError);
          }
        }, 1200 * delayMultiplier++);
      }
      ...
      // Show final message before showdown with enhanced timing
      setTimeout(() => {
      ...
      }, 1200 * delayMultiplier + 1000);
    } else {
      // All cards are already dealt, go straight to showdown
      setTimeout(() => {
        updateInfo("🎯 All cards already dealt! Going to SHOWDOWN...");
    ...
    setTimeout(() => {
      try {
        console.log("[RECOVERY] Attempting emergency card dealing...");
        ...
        // Force round stage to river
        roundStage = 3;
        
        // Update UI
        showCards(false, true);
        updateInfo("Emergency recovery - going to showdown...");
        ...
        updateInfo("Error occurred. Starting new game...");
        setTimeout(() => newGame(), 2000);
      }
    }, 1000);
  }
...
function showAllInPlayerCards() {
  try {
    ...
    // Enhanced all-in detection with multiple criteria
    const allInPlayers = [];
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // Check if player is all-in using multiple criteria
      const isAllIn = chips[i] === 0 || playerStates[i].allIn;
      
      if (isAllIn) {
        allInPlayers.push(i);
        console.log(`[GAME] Player ${i + 1} is all-in, will reveal cards`);
        
        // Mark this player as having revealed cards
        if (!playerStates[i].cardsRevealed) {
          playerStates[i].cardsRevealed = true;
          
          // Add enhanced visual indicator that cards are revealed
          try {
            const playerElement = document.getElementById(`player-${i + 1}`);
            if (playerElement) {
              // Add cards-revealed class
              playerElement.classList.add('cards-revealed');
              
              // Add temporary highlight effect (box shadow only, no scale)
              playerElement.style.boxShadow = '0 0 15px rgba(255, 255, 255, 0.6)';
              ...
              // Add a small "revealed" indicator
              const revealedIndicator = document.createElement('div');
              revealedIndicator.className = 'cards-revealed-indicator';
              revealedIndicator.textContent = '🎴';
              revealedIndicator.style.cssText = `
                position: absolute;
                top: -15px;
                right: -15px;
              ...
              playerElement.appendChild(revealedIndicator);
              
              // Add fadeInScale animation if not already present
              if (!document.querySelector('#fade-in-scale-animation')) {
                const style = document.createElement('style');
                style.id = 'fade-in-scale-animation';
                style.textContent = `
            ...
            console.warn(`[WARNING] UI update for card reveal failed for Player ${i + 1}:`, uiError);
          }
        }
      }
    }
    
    console.log(`[GAME] Revealing cards for ${allInPlayers.length} all-in players: ${allInPlayers.map(p => p + 1).join(', ')}`);
    
    // Update the card display to show revealed cards with enhanced timing
    setTimeout(() => {
      showCards(false, true); // Use a second parameter to show all-in player cards
    }, 300);
    
    // Add dramatic announcement if multiple players are all-in
    if (allInPlayers.length > 1) {
      setTimeout(() => {
        updateInfo(`🎰 ${allInPlayers.length} players are ALL-IN! Cards revealed...`);
      }, 500);
    }
    ...
    // Fallback: try to show cards anyway
    try {
      showCards(false, true);
    } catch (fallbackError) {
      console.error("[ERROR] Fallback card showing also failed:", fallbackError);
    }
  }
}
...
function findNextActivePlayer(startIndex) {
  try {
    ...
    // Check if there are any active players (non-folded)
    const activePlayers = playerStates.filter(p => !p.folded).length;
    if (activePlayers === 0) {
    ...
    // Find next non-folded player who can still act (not all-in or has acted)
    let idx = startIndex;
    let loopCounter = 0; // Safety counter to prevent infinite loops
    
    do {
      idx = (idx - 1 + 4) % 4; // Go backward (counterclockwise) - UTG -> Dealer -> SB -> BB
      loopCounter++;
      ...
      if (loopCounter > 4) {
        ...
        return -1; // No active players found
             ...
             // Skip all-in players who have already acted
             (playerStates[idx].allIn && playerHasActed[idx]));
...
function checkBettingRoundEnd(latestActor) {
  console.log(`[ROUND] checkBettingRoundEnd triggered by Player ${latestActor + 1}`);

  const activePlayers = playerStates.map((p, i) => !p.folded ? i : -1).filter(i => i !== -1);
  
  // Check if all active players have acted (including all-in players)
  const allActed = activePlayers.every(i => {
    // All-in players are considered to have acted
    if (playerStates[i].allIn) {
      return true;
    }
    // Non-all-in players must have actually acted
    return playerHasActed[i];
  });
  
  // Check if all bets are equal (all-in players are considered to have matched)
  const allBetsEqual = activePlayers.every(i => {
    // All-in players are considered to have matched the current bet
    if (playerStates[i].allIn) {
      return true;
    }
    // Non-all-in players must have matched the current bet
    return currentBets[i] === currentBet;
  });

  console.log(`[ROUND] Active: ${activePlayers}, Acted: ${allActed}, Bets Equal: ${allBetsEqual}`);

  if (allActed && allBetsEqual) {
    console.log(`[ROUND] All players have acted and bets match. Ending round.`);

    // Check if AI turn is still pending
    if (window.pendingAITurn) {
      console.warn("[ROUND] AI action still pending. Delaying round end.");
      return;
    }
    ...
    setTimeout(() => {
      nextRound();
    }, 1200); // Short buffer time
  } else {
    goToNextPlayer(latestActor);
  }
}
...
function awardPotToPlayer(playerIdx, reason) {
  try {
    ...
    // Save chips to local storage after awarding pot
    if (typeof saveChipsToLocalStorage === 'function') {
    ...
    if (playerIdx === 3 && typeof updateChipsAfterHand === 'function') {
      if (typeof window !== 'undefined' && window.updateChipsAfterHand) {
        window.updateChipsAfterHand(totalPot);
      } else {
        try {
          updateChipsAfterHand(totalPot);
        } catch (e) {
          console.warn('[SYNC] Could not call updateChipsAfterHand:', e);
        }
      }
    }
    ...
    if (playerIdx === 3) {
      ...
      if (typeof trackHandCompletion === 'function') {
        const handType = getHandTypeFromReason(reason);
        const wasRiverWin = checkRiverWin();
        trackHandCompletion(true, handType, wasRiverWin);
      }
      if (typeof checkAchievements === 'function') {
        checkAchievements(); // Check for win achievements
      }
    } else {
      // User lost - track hand completion
      if (typeof trackHandCompletion === 'function') {
        trackHandCompletion(false);
      }
      if (typeof checkAchievements === 'function') {
        checkAchievements(); // Check for experience achievements
      }
    }
    ...
    const timeout3 = setTimeout(() => {
      try {
        // Reset all-in states for all players before starting new game
        console.log("[AWARD] Resetting all-in states before new game");
        cleanupAllAllInStates();
        
        // Additional cleanup to ensure all-in states are completely reset
        for (let i = 0; i < 4; i++) {
          if (playerStates[i]) {
            playerStates[i].allIn = false;
          }
          playerHasActed[i] = false;
        }
        
        // Rotate positions after hand is complete
        dealerIndex = (dealerIndex + 3) % 4;
        smallBlindIndex = (smallBlindIndex + 3) % 4;
        bigBlindIndex = (bigBlindIndex + 3) % 4;
        underTheGunIndex = (underTheGunIndex + 3) % 4;
        
        // Save positions to local storage
        if (typeof savePositionsToLocalStorage === 'function') {
        ...
        // Emergency recovery - try to start new game without rotation
        setTimeout(() => newGame(), 1000);
      }
    }, 3000);
...
function newGame() {
  // Ensure AI players have at least 500 chips if they are at 0
  ensureMinimumChips(20);
  // Load chips from local storage if available
  if (typeof loadChipsFromLocalStorage === 'function') {
    const savedChips = loadChipsFromLocalStorage();
    if (savedChips && savedChips.length === 4) {
      chips = savedChips;
    } else if (!chips || chips.length !== 4) {
  ...
  // Load positions from local storage if available
  if (typeof loadPositionsFromLocalStorage === 'function') {
    const positionsLoaded = loadPositionsFromLocalStorage();
    if (!positionsLoaded) {
      console.log("[GAME] No saved positions found, using default positions");
    }
  }
  ...
  for (let i = 0; i < 4; i++) {
    ...
    playerStates[i].allIn = false; // Reset all-in status for all players (AI and user)
    playerHasActed[i] = false;
    ...
    if (playerElement) {
      playerElement.classList.remove('folded');
      playerElement.classList.remove('all-in'); // Remove all-in visual class
    }
  ...
  // Reset all-in status comprehensively
  cleanupAllAllInStates();
  
  // Additional all-in state cleanup to ensure complete reset
  console.log("[GAME] Resetting all-in states for all players (AI and user)");
  for (let i = 0; i < 4; i++) {
    // Double-check all-in state is reset
    if (playerStates[i]) {
      playerStates[i].allIn = false;
    }
    ...
    // Remove any remaining all-in visual indicators
    try {
      const playerElement = document.getElementById(`player-${i + 1}`);
      if (playerElement) {
        // Remove all-in class
        ...
        // Remove all-in indicator elements
        const allInIndicator = playerElement.querySelector('.all-in-indicator');
        if (allInIndicator) {
          allInIndicator.remove();
        }
        
        // Reset any inline styles that might have been set for all-in
        playerElement.style.boxShadow = '';
      ...
      console.error(`[ERROR] Failed to cleanup all-in state for player ${i + 1}:`, cleanupError);
    }
  }
  
  // Final verification that all-in states are reset
  let allInResetVerified = true;
  for (let i = 0; i < 4; i++) {
    if (playerStates[i] && playerStates[i].allIn) {
      console.warn(`[WARNING] Player ${i + 1} still has allIn=true after reset, forcing reset`);
      playerStates[i].allIn = false;
      allInResetVerified = false;
    }
  }
  
  if (allInResetVerified) {
    console.log("[GAME] All-in states successfully reset for all players");
  }
  ...
  updatePositionIndicators(dealerIndex, smallBlindIndex, bigBlindIndex);

  // Start the first betting round
  startBettingRound();

  // Ensure AI players have at least 500 chips if they are at 0 (run after all chip loading)
  ensureMinimumChips(20);
...
function ensureMinimumChips(minimumAmount = 20) {
  try {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // If AI player (0,1,2) and chips are 0, reset to 500
      if (i < 3 && chips[i] === 0) {
...
function placeBet(playerIndex, amount) {
  try {
    ...
    // ACHIEVEMENT TRACKING - Track betting for user (player 3)
    if (playerIndex === 3 && actualAmount > 0) {
      if (typeof trackBetting === 'function') {
        trackBetting(actualAmount);
      }
      if (typeof checkAchievements === 'function') {
        checkAchievements(); // Check for betting achievements
      }
    }
...
function getHandScore(cards) {
  try {
    ...
    // Debug output for two pair detection
    console.log(`[DEBUG] getHandScore - counts:`, counts);
    console.log(`[DEBUG] getHandScore - countsArray:`, countsArray);
    console.log(`[DEBUG] getHandScore - cards:`, validCards);

    // Rank the hand from highest to lowest
    if (isRoyalFlush) {
      console.log(`[DEBUG] getHandScore returning 9 (Royal Flush)`);
      return 9;               // Royal flush
    }
    if (isStraightFlush) {
      console.log(`[DEBUG] getHandScore returning 8 (Straight Flush)`);
      return 8;            // Straight flush
    }
    if (countsArray[0] === 4) {
      console.log(`[DEBUG] getHandScore returning 7 (Four of a Kind)`);
      return 7;       // Four of a kind
    }
    if (countsArray[0] === 3 && countsArray[1] >= 2) {
      console.log(`[DEBUG] getHandScore returning 6 (Full House)`);
      return 6; // Full house
    }
    if (isFlush) {
      console.log(`[DEBUG] getHandScore returning 5 (Flush)`);
      return 5;                    // Flush
    }
    if (isStraight) {
      console.log(`[DEBUG] getHandScore returning 4 (Straight)`);
      return 4;                 // Straight
    }
    if (countsArray[0] === 3) {
      console.log(`[DEBUG] getHandScore returning 3 (Three of a Kind)`);
      return 3;       // Three of a kind
    }
    if (countsArray[0] === 2 && countsArray[1] === 2) {
      console.log(`[DEBUG] getHandScore returning 2 (Two Pair)`);
      return 2; // Two pair
    }
    if (countsArray[0] === 2) {
      console.log(`[DEBUG] getHandScore returning 1 (One Pair)`);
      return 1;       // One pair
    }

    console.log(`[DEBUG] getHandScore returning 0 (High Card)`);
    return 0; // High card
  } catch (error) {
...
function aiTakeTurn(aiIndex) {
    ...
    try {
        ...
        // Post-flop evaluation
            ...
            handStrength = Math.min(handScore / 9, 1);
        ...
        if (humanMemory.length > 3) {
            ...
            if (humanAggressionLevel > 0.7) {
                if (decision === 'call' && perceivedHandStrength > 0.6 && Math.random() < 0.4) {
                    decision = 'raise';
                    betAmount = determineBetSize() * 1.2; // Bigger raises against aggressive players
...
function evaluateHandStrength(playerCards, communityCards) {
  try {
    ...
    try {
      ...
      // Calculate proper hand strength based on actual poker hand rankings
      let baseStrength = 0;
      
      // Map hand scores to realistic strength values
      switch (score) {
        case 9: // Royal Flush
          baseStrength = 0.99;
          break;
        ...
        case 3: // Three of a Kind
          ...
          break;
        case 2: // Two Pair
          // For two pair, strength depends on the pair ranks
          baseStrength = calculateTwoPairStrength(fullHand);
          break;
        case 1: // One Pair
          // For pairs, strength depends on the pair rank
          baseStrength = calculatePairStrength(fullHand);
          break;
        case 0: // High Card
          // For high card, strength depends on the high card rank
          baseStrength = calculateHighCardStrength(fullHand);
          break;
        default:
          baseStrength = 0.1;
      }
      ...
      if (commCards.length >= 4) {
        // On turn or river, slightly discount made hands (less room for improvement)
        adjustedStrength = Math.pow(baseStrength, 1.1);
      } else {
        // On flop, value made hands more (they can still be outdrawn, but are still strong)
      ...
      console.log(`[GAME] Hand evaluated: score ${score}, base strength ${baseStrength.toFixed(2)}, adjusted ${adjustedStrength.toFixed(2)}`);
      
      // Add detailed debugging for the specific case mentioned
      if (score === 2) { // Two pair
        console.log(`[DEBUG] Two pair detected! Cards: ${fullHand.join(', ')}`);
        console.log(`[DEBUG] Player cards: ${playerCards.join(', ')}`);
        console.log(`[DEBUG] Community cards: ${commCards.join(', ')}`);
      }
...
/**
 * Calculates the strength of a pair based on the pair rank
 * @param {string[]} cards - All cards (hole + community)
 * @returns {number} - Pair strength from 0 to 1
 */
function calculatePairStrength(cards) {
  try {
    const values = '23456789TJQKA';
    const cardValues = cards.map(card => card && card.length >= 1 ? values.indexOf(card[0]) : -1)
                           .filter(idx => idx !== -1);
    ...
    // Find the pair (value with count 2)
    const pairValue = Object.entries(valueCounts).find(([val, count]) => count === 2);
    
    if (!pairValue) {
      console.warn("[WARNING] No pair found in calculatePairStrength");
      return 0.3; // Default pair strength
    }
    
    const pairRank = parseInt(pairValue[0]);
    
    // Calculate pair strength based on rank (higher rank = stronger pair)
    // Scale from 0.4 (pair of 2s) to 0.8 (pair of aces)
    const pairStrength = 0.4 + (pairRank / 12) * 0.4;
    
    // Add bonus for high kickers
    const kickers = cardValues.filter(val => val !== pairRank).sort((a, b) => b - a);
    const kickerBonus = kickers.length > 0 ? (kickers[0] / 12) * 0.1 : 0;
    
    return Math.min(pairStrength + kickerBonus, 0.85);
  } catch (error) {
    console.error("[ERROR] Exception in calculatePairStrength:", error);
    return 0.4; // Default pair strength
  }
}

/**
 * Calculates the strength of a high card hand based on the high card rank
 * @param {string[]} cards - All cards (hole + community)
 * @returns {number} - High card strength from 0 to 1
 */
function calculateHighCardStrength(cards) {
  try {
    const values = '23456789TJQKA';
    const cardValues = cards.map(card => card && card.length >= 1 ? values.indexOf(card[0]) : -1)
                           .filter(idx => idx !== -1);
    
    if (cardValues.length === 0) return 0.1;
    
    // Sort values high to low
    cardValues.sort((a, b) => b - a);
    
    // Calculate strength based on top 3 cards
    const topCard = cardValues[0];
    const secondCard = cardValues[1] || 0;
    const thirdCard = cardValues[2] || 0;
    
    // Base strength on top card (0.1 for 2, 0.4 for ace)
    const baseStrength = 0.1 + (topCard / 12) * 0.3;
    
    // Add small bonuses for second and third cards
    const secondBonus = (secondCard / 12) * 0.05;
    const thirdBonus = (thirdCard / 12) * 0.02;
    
    return Math.min(baseStrength + secondBonus + thirdBonus, 0.45);
  } catch (error) {
    console.error("[ERROR] Exception in calculateHighCardStrength:", error);
    return 0.1; // Default high card strength
  }
}

/**
 * Calculates the strength of a two pair hand based on the pair ranks
 * @param {string[]} cards - All cards (hole + community)
 * @returns {number} - Two pair strength from 0 to 1
 */
function calculateTwoPairStrength(cards) {
  try {
    const values = '23456789TJQKA';
    const cardValues = cards.map(card => card && card.length >= 1 ? values.indexOf(card[0]) : -1)
                           .filter(idx => idx !== -1);
    ...
    // Find the two pairs (values with count 2)
    ...
    if (pairs.length < 2) {
      console.warn("[WARNING] Not enough pairs found in calculateTwoPairStrength:", pairs);
      return 0.5; // Default two pair strength
    }
    
    const higherPair = pairs[0];
    const lowerPair = pairs[1];
    
    // Calculate base strength based on the higher pair (0.6 for 22, 0.85 for AA)
    const baseStrength = 0.6 + (higherPair / 12) * 0.25;
    
    // Add bonus for the lower pair (0.05 for 22, 0.15 for AA)
    const lowerPairBonus = (lowerPair / 12) * 0.1;
    
    // Find the kicker (highest card that's not in either pair)
    const kickers = cardValues.filter(val => val !== higherPair && val !== lowerPair).sort((a, b) => b - a);
    const kickerBonus = kickers.length > 0 ? (kickers[0] / 12) * 0.05 : 0;
    
    const totalStrength = baseStrength + lowerPairBonus + kickerBonus;
    
    console.log(`[DEBUG] Two pair calculation: higher pair ${higherPair}, lower pair ${lowerPair}, base ${baseStrength.toFixed(2)}, lower bonus ${lowerPairBonus.toFixed(2)}, kicker bonus ${kickerBonus.toFixed(2)}, total ${totalStrength.toFixed(2)}`);
    
    return Math.min(totalStrength, 0.9);
  } catch (error) {
    console.error("[ERROR] Exception in calculateTwoPairStrength:", error);
    return 0.6; // Default two pair strength
  }
}
...
function applyHumanLikeAdjustments(aiIndex, baseDecision, handStrength) {
  try {
    ...
    if (hour < 6 || hour >= 23) {
      ...
      // Night owls get looser late night
...
function moveChipsFromPotToPlayer(playerIndex) {
  try {
    ...
    // Check if chip animations are enabled in settings
    if (!getSetting('chip-animations')) {
      console.log('[ANIMATION] Chip animations disabled, skipping animation');
      return true;
    }
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            // Set target position with slight randomness
...
// Save the user's chips to local storage
function saveChipsToLocalStorage() {
    try {
        const userId = getUserIdFromURL();
        if (userId) {
            localStorage.setItem(`pokerChips_${userId}`, JSON.stringify(chips));
        } else {
            // Fallback to old method if no user ID
            localStorage.setItem('pokerChips', JSON.stringify(chips));
        }
    } catch (e) {
        console.error('Failed to save chips to local storage:', e);
    }
}
window.saveChipsToLocalStorage = saveChipsToLocalStorage;
...
function loadChipsFromLocalStorage() {
  try {
      ...
      let savedChips;
      if (userId) {
          savedChips = localStorage.getItem(`pokerChips_${userId}`);
      } else {
          // Fallback to old method if no user ID
          savedChips = localStorage.getItem('pokerChips');
      }
      if (savedChips) {
...
window.loadChipsFromLocalStorage = loadChipsFromLocalStorage;

// Save position indices to local storage
function savePositionsToLocalStorage() {
    try {
        ...
        if (userId) {
            localStorage.setItem(`pokerPositions_${userId}`, JSON.stringify(positions));
        } else {
            // Fallback to old method if no user ID
            localStorage.setItem('pokerPositions', JSON.stringify(positions));
        }
        console.log(`[GAME] Saved positions to local storage: Dealer=${dealerIndex}, SB=${smallBlindIndex}, BB=${bigBlindIndex}, UTG=${underTheGunIndex}`);
    } catch (e) {
        console.error('Failed to save positions to local storage:', e);
    }
}
window.savePositionsToLocalStorage = savePositionsToLocalStorage;

// Load position indices from local storage
function loadPositionsFromLocalStorage() {
    try {
        ...
        if (userId) {
            savedPositions = localStorage.getItem(`pokerPositions_${userId}`);
        } else {
            // Fallback to old method if no user ID
            savedPositions = localStorage.getItem('pokerPositions');
        }
        
        if (savedPositions) {
            const positions = JSON.parse(savedPositions);
            
            // Validate the loaded positions
            if (positions.dealerIndex !== undefined && positions.smallBlindIndex !== undefined && 
                positions.bigBlindIndex !== undefined && positions.underTheGunIndex !== undefined) {
                
                dealerIndex = positions.dealerIndex;
                smallBlindIndex = positions.smallBlindIndex;
                bigBlindIndex = positions.bigBlindIndex;
                underTheGunIndex = positions.underTheGunIndex;
                
                console.log(`[GAME] Loaded positions from local storage: Dealer=${dealerIndex}, SB=${smallBlindIndex}, BB=${bigBlindIndex}, UTG=${underTheGunIndex}`);
                return true;
            }
        }
    } catch (e) {
        console.error('Failed to load positions from local storage:', e);
    }
    return false;
}
window.loadPositionsFromLocalStorage = loadPositionsFromLocalStorage;

/**
 * Displays advanced hand strength, pot odds, and betting information for the human player
 * This function calculates and updates the hand strength display with real poker concepts
 */
function displayPlayerInfo() {
  try {
    // Get the display element
    const displayElement = document.getElementById('hand-strength-display');
    if (!displayElement) {
      console.warn("[WARNING] Hand strength display element not found");
      return;
    }

    // Check if hand strength display is enabled in settings
    if (!getSetting('show-hand-strength')) {
    ...
    // Check if player has cards
    if (!players[3] || players[3].length < 2) {
      displayElement.textContent = "Waiting for cards...";
      return;
    }

    // Calculate hand strength (now true equity)
    let handStrength = 0;
    ...
    if (communityCards.length === 0) {
      // Pre-flop evaluation
      handStrength = evaluatePreFlopEquity(players[3]);
      handRank = getPreFlopHandRank(players[3]);
      equity = Math.round(handStrength * 100);
    } else {
      // Post-flop evaluation
      handStrength = evaluateHandStrength(players[3], communityCards);
      const fullHand = [...players[3], ...communityCards];
      handRank = getHandRank(fullHand);
      equity = Math.round(handStrength * 100);
    }
    ...
    const toCall = currentBet - currentBets[3];
    const potOdds = toCall > 0 ? (potSize / toCall) : Infinity;
    const potOddsPercent = toCall > 0 ? Math.round((toCall / (potSize + toCall)) * 100) : 0;

    // Clarified pot odds display
    let potOddsText = toCall > 0 ? `Pot odds: ${potOddsPercent}% (Call $${toCall} to win $${potSize})` : 'No bet to call';

    // Effective stack sizes
    const playerStack = chips[3];
    const effectiveStack = Math.min(playerStack, Math.max(...chips.filter((c, i) => !playerStates[i].folded && i !== 3)));
    const stackToPotRatio = potSize > 0 ? (effectiveStack / potSize) : 0;

    // Generate betting recommendation
    const recommendation = getBettingRecommendation(handStrength, potOdds, toCall, stackToPotRatio, communityCards.length);

    // Format the display with beginner-friendly information
    let displayText = "";
    if (communityCards.length === 0) {
      displayText = `${handRank} | ${equity}% equity | ${potOddsText}`;
    } else {
      const streetName = communityCards.length === 3 ? "After Flop" : 
                        communityCards.length === 4 ? "After Turn" : "After River";
      displayText = `${handRank} | ${equity}% equity | ${streetName} | ${recommendation} | ${potOddsText}`;
    }
    if (toCall > 0) {
      displayText += ` | Need $${toCall} to call`;
    }
    displayElement.textContent = displayText;
    console.log(`[UI] Advanced player info updated - ${displayText}`);
  } catch (error) {
    console.error("[ERROR] Failed to display advanced player info:", error);
    try {
      const displayElement = document.getElementById('hand-strength-display');
      if (displayElement) {
        displayElement.textContent = "Analyzing hand...";
      }
    } catch (fallbackError) {
      console.error("[ERROR] Even fallback display failed:", fallbackError);
    }
  }
}

/**
 * Gets the pre-flop hand ranking name
 */
function getPreFlopHandRank(cards) {
  try {
    ...
    if (isPair) {
      const pairNames = {14: "Aces", 13: "Kings", 12: "Queens", 11: "Jacks", 10: "Tens", 9: "Nines", 8: "Eights", 7: "Sevens", 6: "Sixes", 5: "Fives", 4: "Fours", 3: "Threes", 2: "Twos"};
      return pairNames[val1] || "Pair";
    }
    
    const highCard = Math.max(val1, val2);
    const lowCard = Math.min(val1, val2);
    const highNames = {14: "Ace", 13: "King", 12: "Queen", 11: "Jack", 10: "Ten"};
    const lowNames = {14: "Ace", 13: "King", 12: "Queen", 11: "Jack", 10: "Ten"};
    
    const highName = highNames[highCard] || highCard;
    const lowName = lowNames[lowCard] || lowCard;
    
    return `${highName}-${lowName}${isSuited ? ' suited' : ''}`;
  } catch (error) {
    console.error("[ERROR] Failed to get pre-flop hand rank:", error);
    return "Unknown";
  }
}

/**
 * Gets the post-flop hand ranking name
 */
function getHandRank(cards) {
  try {
    ...
    return rankNames[handScore] || "Unknown";
  } catch (error) {
    console.error("[ERROR] Failed to get hand rank:", error);
    return "Unknown";
  }
}

/**
 * Generates betting recommendation based on hand strength and situation
*/
function getBettingRecommendation(handStrength, potOdds, toCall, stackToPotRatio, communityCardsCount) {
  try {
    if (toCall === 0) {
      // No bet to call - can check or bet
      if (handStrength > 0.7) return "Strong hand";
      if (handStrength > 0.5) return "Good hand";
      if (handStrength > 0.3) return "Okay hand";
      return "Weak hand";
    } else {
      // There's a bet to call
      const potOddsRatio = potOdds;
      const equity = handStrength;
      
      if (equity > 0.8) return "Very strong";
      if (equity > 0.6) return "Strong";
      if (equity > potOddsRatio / (potOddsRatio + 1)) return "Good odds";
      if (stackToPotRatio < 3 && equity > 0.3) return "Short stack";
      return "Weak hand";
    }
  } catch (error) {
    console.error("[ERROR] Failed to get betting recommendation:", error);
    return "Check";
  }
}
...
function updateSliderMin() {
  try {
    ...
    console.log(`[UI] Slider updated: min=$${slider.min}, max=$${slider.max}, value=$${slider.value}, mode=${slider.mode}`);
    return true;
  } catch (error) {
...
//Add localStorage hands played logic for cross-page tracking
function incrementHandsPlayed() {
  console.log("[DEBUG] incrementHandsPlayed called");

  // Safe fallback to avoid errors if UI function doesn't exist
  if (typeof updateUserStatsUI === 'function') {
    updateUserStatsUI('handsPlayed', 1);
  }
}
...
function getHandsPlayed() {
  const userId = getUserIdFromURL();
  return localStorage.getItem(`handsPlayed_${userId}`) || '0';
}

// Add localStorage win logic for cross-page tracking
function incrementWins() {
  const userId = getUserIdFromURL();
  let wins = parseInt(localStorage.getItem(`wins_${userId}`) || '0', 10);
  wins += 1;
  localStorage.setItem(`wins_${userId}`, wins);
  // Update streak
  let streak = parseInt(localStorage.getItem(`consecutiveWins_${userId}`) || '0', 10);
  streak += 1;
  localStorage.setItem(`consecutiveWins_${userId}`, streak);
  updateUserStatsUI();
}

function getWins() {
  const userId = getUserIdFromURL();
  return localStorage.getItem(`wins_${userId}`) || '0';
}

// Add localStorage loss logic for cross-page tracking
function incrementLosses() {
  const userId = getUserIdFromURL();
  let losses = parseInt(localStorage.getItem(`losses_${userId}`) || '0', 10);
  losses += 1;
  localStorage.setItem(`losses_${userId}`, losses);
  // Reset streak
...
function getLosses() {
  const userId = getUserIdFromURL ? getUserIdFromURL() : null;
  const key = userId ? `losses_${userId}` : 'losses';
  return parseInt(localStorage.getItem(key) || '0', 10);
}

// === RECENT ACTIVITY LOGGING (LOCAL) ===
function addRecentActivity(description, type) {
  try {
    const userId = getUserIdFromURL ? getUserIdFromURL() : null;
    const key = userId ? `recentActivities_${userId}` : 'recentActivities';
    let activities = [];
    try {
    ...
    // Update profile statistics when win/loss activities are logged
    if (type === 'win' || type === 'loss') {
      updateProfileStats(type);
    }
    
    // If activityList exists, update UI
    if (typeof window !== 'undefined' && window.renderRecentActivities) {
      window.renderRecentActivities();
    }
  } catch (e) {
    console.error('[ACTIVITY] Failed to add recent activity:', e);
  }
}

/**
 * Updates the user's profile statistics (wins/losses) based on activity type
 * @param {string} activityType - The type of activity ('win' or 'loss')
 */
function updateProfileStats(activityType) {
  try {
    const userId = getUserIdFromURL ? getUserIdFromURL() : null;
    
    if (activityType === 'win') {
      // Update wins in profile
      if (typeof incrementWins === 'function') {
        incrementWins();
        console.log('[PROFILE] Updated wins count for user profile');
      } else {
        // Fallback: manually update localStorage wins
        const key = userId ? `wins_${userId}` : 'wins';
        let wins = parseInt(localStorage.getItem(key) || '0', 10);
        wins += 1;
        localStorage.setItem(key, wins);
        console.log('[PROFILE] Manually updated wins count:', wins);
      }
      
      // Also update Firestore if available and user ID exists
      if (userId && typeof window !== 'undefined' && window.firebase && window.firebase.firestore) {
        try {
          ...
          if (db && typeof db.collection === 'function') {
            db.collection('users').doc(userId).update({
              ...
              console.log('[PROFILE] Updated wins in Firestore');
            }).catch(error => {
              console.error('[PROFILE] Failed to update wins in Firestore:', error);
            });
          }
        } catch (firestoreError) {
          console.log('[PROFILE] Firestore not available for wins update');
        }
      }
      
    } else if (activityType === 'loss') {
      // Update losses in profile
      if (typeof incrementLosses === 'function') {
        incrementLosses();
        console.log('[PROFILE] Updated losses count for user profile');
      } else {
        // Fallback: manually update localStorage losses
        const key = userId ? `losses_${userId}` : 'losses';
        let losses = parseInt(localStorage.getItem(key) || '0', 10);
        losses += 1;
        localStorage.setItem(key, losses);
        console.log('[PROFILE] Manually updated losses count:', losses);
      }
      ...
      if (userId && typeof window !== 'undefined' && window.firebase && window.firebase.firestore) {
        try {
          ...
          if (db && typeof db.collection === 'function') {
            db.collection('users').doc(userId).update({
              losses: window.firebase.firestore.FieldValue.increment(1)
            }).then(() => {
              console.log('[PROFILE] Updated losses in Firestore');
            }).catch(error => {
              console.error('[PROFILE] Failed to update losses in Firestore:', error);
            });
          }
        } catch (firestoreError) {
          console.log('[PROFILE] Firestore not available for losses update');
        }
      }
    }
    
    // Update the profile display if we're on the profile page
    if (typeof window !== 'undefined') {
      // Update wins display
      if (typeof updateWinsDisplay === 'function') {
        updateWinsDisplay();
      } else {
        ...
        if (userWinsElement) {
          const key = userId ? `wins_${userId}` : 'wins';
          const wins = localStorage.getItem(key) || '0';
          userWinsElement.textContent = wins;
        }
      }
      
      // Update losses display
      if (typeof updateLossesDisplay === 'function') {
        updateLossesDisplay();
      } else {
        // Manual display update
        ...
        if (userLossesElement) {
          const key = userId ? `losses_${userId}` : 'losses';
          const losses = localStorage.getItem(key) || '0';
          userLossesElement.textContent = losses;
        }
      }
    }
 ...
 */
function getRecentActivitiesFromStorage(limit = 20) {
  const userId = getUserIdFromURL ? getUserIdFromURL() : null;
  const key = userId ? `recentActivities_${userId}` : 'recentActivities';
  let activities = [];
  try {
...
window.getRecentActivitiesFromStorage = getRecentActivitiesFromStorage;

function renderRecentActivities() {
  try {
    const activities = getRecentActivitiesFromStorage(20);
    const activityList = document.getElementById('poker-activity-list');
    ...
    console.error('[ACTIVITY] Failed to render recent activities:', e);
  }
}
...
function playerAction(action) {
  try {
    ...
    if (action === 'bet-slider') {
      ...
      if (playerStates[currentPlayer].allIn) {
        console.warn(`[WARNING] Player ${currentPlayer + 1} is all-in and cannot fold.`);
        return;
      }
  ...
  function handlePlayerBetSlider() {
    try {
      ...
      if (actionType === 'bet') {
        ...
        // Track betting for achievements
        if (typeof trackBetting === 'function') {
          trackBetting(betAmount);
          console.log(`[ACHIEVEMENT] Tracked bet of $${betAmount}`);
        }
        
        // Log activity for bet
        addRecentActivity(`You bet $${betAmount}`, 'account');
        ...
        if (handleRaise(currentPlayer, betAmount)) {
          ...
          // Track betting for achievements (raise amount is the additional amount)
          if (typeof trackBetting === 'function') {
            trackBetting(betAmount);
            console.log(`[ACHIEVEMENT] Tracked raise of $${betAmount}`);
          }
          
          // Log activity for raise
          addRecentActivity(`You raised $${betAmount}`, 'account');
  ...
  function handlePlayerCall() {
    try {
      ...
      if (handleCall(currentPlayer)) {
        ...
        addRecentActivity(`You called $${toCall}`, 'account');
...
/**
 * Rotates the dealer and blind positions counterclockwise
 */
function rotatePositions() {
  console.log("[GAME] Rotating positions counterclockwise");
  
  // Rotate positions counterclockwise (to the left)
  dealerIndex = (dealerIndex + 3) % 4;  // +3 is equivalent to -1 in modulo 4
  smallBlindIndex = (smallBlindIndex + 3) % 4;
  bigBlindIndex = (bigBlindIndex + 3) % 4;
  underTheGunIndex = (underTheGunIndex + 3) % 4;
...
function updatePositionIndicators(dealerIndex, smallBlindIndex, bigBlindIndex) {
  console.log(`[UI] Updating position indicators - Dealer: ${dealerIndex + 1}, SB: ${smallBlindIndex + 1}, BB: ${bigBlindIndex + 1}, UTG: ${underTheGunIndex + 1}`);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (playerElement) {
      ...
      const utgBtn = playerElement.querySelector('.under-the-gun');
      ...
      if (utgBtn) utgBtn.classList.remove('active');
    }
  }
  ...
  const utgElement = document.getElementById(`player-${underTheGunIndex + 1}`);
  ...
  if (utgElement) {
    const utgBtn = utgElement.querySelector('.under-the-gun');
    if (utgBtn) utgBtn.classList.add('active');
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  initPhases.forEach(phase => {
    setTimeout(() => {
      try {
        ...
        console.error(`[INIT-ERROR] Failed to initialize ${phase.name}:`, e);
      }
    }, phase.delay);
  });
  ...
  function startNewGame() {
    ...
    // Set user-specific ongoing game flag
    const userId = getUserIdFromURL();
    if (userId) {
      localStorage.setItem(`hasOngoingGame_${userId}`, 'true');
      console.log(`[INIT] Set hasOngoingGame_${userId} to true`);
    }
    
    // Save default positions if no saved positions exist
    if (typeof savePositionsToLocalStorage === 'function' && typeof loadPositionsFromLocalStorage === 'function') {
      const positionsLoaded = loadPositionsFromLocalStorage();
      if (!positionsLoaded) {
        console.log("[INIT] No saved positions found, saving default positions");
        savePositionsToLocalStorage();
      }
    }
...
function setupBasicUI() {
  ...
  if (typeof renderRecentActivities === 'function') {
    try {
      renderRecentActivities();
      console.log("[INIT] Recent activity panel initialized");
    } catch (e) {
      console.error("[INIT-ERROR] Failed to initialize recent activity panel:", e);
    }
  }
...
function setupKeyboardShortcuts() {
  ...
  if (!getSetting('enable-keyboard-shortcuts')) {
    console.log('[KEYBOARD] Keyboard shortcuts disabled in settings');
    return;
  }
...
function setupGameControls() {
  ...
  const showTooltips = getSetting('show-tooltips');
  ...
  if (menuButton) {
    ...
    newMenuButton.addEventListener('click', () => {
      console.log("[UI] Return to menu button clicked");
      
      // Show confirmation dialog
      const confirmed = confirm("Are you sure you want to return to the main menu? Your current game progress will be lost.");
      
      if (confirmed) {
        // Navigate back to the main menu (app_index.html)
        window.location.href = 'app_index.html';
      }
    });
    
    if (showTooltips) {
      newMenuButton.title = 'Return to Main Menu';
    }
  }
  ...
  if (betButton && showTooltips) {
    betButton.title = 'Bet/Raise (B)';
  }
...
/**
 * Cleans up all-in state and visual indicators for a player
 * @param {number} playerIndex - Index of the player to clean up
 */
function cleanupAllInState(playerIndex) {
  try {
    // Validate player index
    if (playerIndex < 0 || playerIndex > 3) {
      console.error(`[ERROR] Invalid player index in cleanupAllInState: ${playerIndex}`);
      return;
    }
    
    // Reset all-in state in playerStates
    if (playerStates[playerIndex]) {
      playerStates[playerIndex].allIn = false;
    }
    
    // Reset action flag for this player
    playerHasActed[playerIndex] = false;
    ...
    if (playerElement) {
      ...
      // Remove all-in indicator
      const allInIndicator = playerElement.querySelector('.all-in-indicator');
      if (allInIndicator) {
        allInIndicator.remove();
      }
      
      // Remove cards revealed indicator
      const cardsRevealedIndicator = playerElement.querySelector('.cards-revealed-indicator');
      if (cardsRevealedIndicator) {
        cardsRevealedIndicator.remove();
      }
      
      // Remove cards-revealed class
      playerElement.classList.remove('cards-revealed');
    ...
    console.log(`[UI] Cleaned up all-in state for Player ${playerIndex + 1}`);
  } catch (error) {
    console.error(`[ERROR] Failed to cleanup all-in state for Player ${playerIndex + 1}:`, error);
  }
}
/**
 * Cleans up all-in state for all players
 */
function cleanupAllAllInStates() {
  for (let i = 0; i < 4; i++) {
    cleanupAllInState(i);
  }
}
...
let achievementTracking = {
    ...
    foldDisciplineStreak: 0, // Track consecutive folds of big hands
    daysWithoutAllIn: 0,     // Track days without going all-in
    mindfulBreaks: 0,        // Track mindful breaks taken
    lastAllInDate: null,     // Track when user last went all-in
    bigHandFolds: 0          // Track folds of big hands (high bet amounts)
};
...
function initializeAchievementTracking() {
    ...
    achievementTracking.maxBetInHand = parseInt(localStorage.getItem(`maxBetInHand_${userId}`) || '0', 10);
    achievementTracking.consecutiveWins = parseInt(localStorage.getItem(`consecutiveWins_${userId}`) || '0', 10);
    achievementTracking.totalWins = parseInt(localStorage.getItem(`wins_${userId}`) || '0', 10);
    achievementTracking.totalHandsPlayed = parseInt(localStorage.getItem(`handsPlayed_${userId}`) || '0', 10);
    achievementTracking.riverWins = parseInt(localStorage.getItem(`riverWins_${userId}`) || '0', 10);
    achievementTracking.foldStreak = parseInt(localStorage.getItem(`foldStreak_${userId}`) || '0', 10);
    achievementTracking.consecutiveFolds = parseInt(localStorage.getItem(`consecutiveFolds_${userId}`) || '0', 10);
    achievementTracking.lowestChipsReached = parseInt(localStorage.getItem(`lowestChipsReached_${userId}`) || '1000', 10);
    
    // Load RECOVERY PATH tracking data
    achievementTracking.foldDisciplineStreak = parseInt(localStorage.getItem(`foldDisciplineStreak_${userId}`) || '0', 10);
    achievementTracking.daysWithoutAllIn = parseInt(localStorage.getItem(`daysWithoutAllIn_${userId}`) || '0', 10);
    achievementTracking.mindfulBreaks = parseInt(localStorage.getItem(`mindfulBreaks_${userId}`) || '0', 10);
    achievementTracking.bigHandFolds = parseInt(localStorage.getItem(`bigHandFolds_${userId}`) || '0', 10);
    
    // Load last all-in date
    const lastAllInDateStr = localStorage.getItem(`lastAllInDate_${userId}`);
    achievementTracking.lastAllInDate = lastAllInDateStr ? new Date(lastAllInDateStr) : null;
    
    // Load hand types won
    try {
        const handTypesData = localStorage.getItem(`handTypesWon_${userId}`);
        achievementTracking.handTypesWon = handTypesData ? JSON.parse(handTypesData) : {};
    } catch (e) {
        achievementTracking.handTypesWon = {};
    }
    
    // Make recovery path variables available globally for main app
    window.foldDisciplineStreak = achievementTracking.foldDisciplineStreak;
    window.daysWithoutAllIn = achievementTracking.daysWithoutAllIn;
    window.mindfulBreaks = achievementTracking.mindfulBreaks;
    
    console.log('[ACHIEVEMENT] Tracking initialized:', achievementTracking);
}

// Save achievement tracking data
function saveAchievementTracking() {
    const userId = getUserIdFromURL();
    if (!userId) return;
    
    localStorage.setItem(`maxBetInHand_${userId}`, achievementTracking.maxBetInHand);
    localStorage.setItem(`consecutiveWins_${userId}`, achievementTracking.consecutiveWins);
    localStorage.setItem(`riverWins_${userId}`, achievementTracking.riverWins);
    localStorage.setItem(`foldStreak_${userId}`, achievementTracking.foldStreak);
    localStorage.setItem(`consecutiveFolds_${userId}`, achievementTracking.consecutiveFolds);
    localStorage.setItem(`lowestChipsReached_${userId}`, achievementTracking.lowestChipsReached);
    localStorage.setItem(`handTypesWon_${userId}`, JSON.stringify(achievementTracking.handTypesWon));
    
    // Save RECOVERY PATH tracking data
    localStorage.setItem(`foldDisciplineStreak_${userId}`, achievementTracking.foldDisciplineStreak);
    localStorage.setItem(`daysWithoutAllIn_${userId}`, achievementTracking.daysWithoutAllIn);
    localStorage.setItem(`mindfulBreaks_${userId}`, achievementTracking.mindfulBreaks);
    localStorage.setItem(`bigHandFolds_${userId}`, achievementTracking.bigHandFolds);
    
    // Save last all-in date
    if (achievementTracking.lastAllInDate) {
        localStorage.setItem(`lastAllInDate_${userId}`, achievementTracking.lastAllInDate.toISOString());
    }
    
    // Make tracking data available globally for the main app
    window.achievementTracking = achievementTracking;
    window.foldDisciplineStreak = achievementTracking.foldDisciplineStreak;
    window.daysWithoutAllIn = achievementTracking.daysWithoutAllIn;
    window.mindfulBreaks = achievementTracking.mindfulBreaks;
}
...
// Track betting for achievements
function trackBetting(amount) {
    if (amount > achievementTracking.maxBetInHand) {
        achievementTracking.maxBetInHand = amount;
        console.log(`[ACHIEVEMENT] New max bet recorded: $${amount}`);
    }
    
    achievementTracking.currentHandBet = Math.max(achievementTracking.currentHandBet, amount);
    saveAchievementTracking();
    
    // Trigger achievement check for betting achievements
    if (typeof trackGameEvent === 'function') {
        trackGameEvent('bet', { amount: amount });
    }
}
...
function trackHandCompletion(isWin, handType = null, wasRiverWin = false) {
    ...
    if (isWin) {
        ...
        achievementTracking.consecutiveFolds = 0; // Reset fold streak on win
        
        // Track river wins
        if (wasRiverWin) {
            achievementTracking.riverWins++;
            achievementTracking.wonWithRiver = true;
            console.log('[ACHIEVEMENT] River win recorded!');
        }
        
        // Track hand types won
        if (handType) {
            achievementTracking.handTypesWon[handType] = true;
            console.log(`[ACHIEVEMENT] Hand type won: ${handType}`);
        }
        
        // Track lowest chips reached
        const currentChips = chips[3]; // User is player 3
        if (currentChips < achievementTracking.lowestChipsReached) {
            achievementTracking.lowestChipsReached = currentChips;
        }
        
    } else {
        achievementTracking.consecutiveWins = 0;
    }
    
    // Reset current hand bet
    achievementTracking.currentHandBet = 0;
    
    saveAchievementTracking();
    
    // Trigger achievement check
    if (typeof trackGameEvent === 'function') {
        trackGameEvent(isWin ? 'win' : 'loss', { 
            handType: handType,
            wasRiverWin: wasRiverWin,
            chipsWon: isWin ? pot : 0
        });
    }
}
...
// Track folding
function trackFold() {
    achievementTracking.consecutiveFolds++;
    achievementTracking.consecutiveWins = 0; // Reset win streak on fold
    
    if (achievementTracking.consecutiveFolds > achievementTracking.foldStreak) {
        achievementTracking.foldStreak = achievementTracking.consecutiveFolds;
        console.log(`[ACHIEVEMENT] New fold streak: ${achievementTracking.foldStreak}`);
    }
    ...
    // Trigger achievement check
    if (typeof trackGameEvent === 'function') {
        trackGameEvent('fold', { streak: achievementTracking.consecutiveFolds });
    }
}

// Track fold discipline (folding big hands)
function trackFoldDiscipline(betAmount) {
    // Consider a "big hand" if the bet is 100+ chips or 20%+ of user's chips
    const userChips = chips[3]; // User is player 3
    const isBigHand = betAmount >= 100 || betAmount >= (userChips * 0.2);
    
    if (isBigHand) {
        achievementTracking.bigHandFolds++;
        achievementTracking.foldDisciplineStreak++;
        console.log(`[ACHIEVEMENT] Big hand fold recorded! Bet: $${betAmount}, Streak: ${achievementTracking.foldDisciplineStreak}`);
        
        // Update global variable for main app
        window.foldDisciplineStreak = achievementTracking.foldDisciplineStreak;
        ...
        // Trigger achievement check
        if (typeof trackGameEvent === 'function') {
            trackGameEvent('fold-discipline', { 
                betAmount: betAmount, 
                streak: achievementTracking.foldDisciplineStreak 
            });
        }
    } else {
        // Reset streak if it's not a big hand
        achievementTracking.foldDisciplineStreak = 0;
        window.foldDisciplineStreak = 0;
        saveAchievementTracking();
    }
}

// Track all-in actions
function trackAllIn() {
    const now = new Date();
    achievementTracking.lastAllInDate = now;
    
    // Reset days without all-in
    achievementTracking.daysWithoutAllIn = 0;
    window.daysWithoutAllIn = 0;
    
    console.log('[ACHIEVEMENT] All-in recorded, resetting days without all-in');
    
    saveAchievementTracking();
    
    // Trigger achievement check
    if (typeof trackGameEvent === 'function') {
        trackGameEvent('all-in', { date: now });
    }
}

// Track mindful breaks
function trackMindfulBreak() {
    achievementTracking.mindfulBreaks++;
    window.mindfulBreaks = achievementTracking.mindfulBreaks;
    
    console.log(`[ACHIEVEMENT] Mindful break recorded! Total: ${achievementTracking.mindfulBreaks}`);
    
    saveAchievementTracking();
    
    // Trigger achievement check
    if (typeof trackGameEvent === 'function') {
        trackGameEvent('mindful-break', { total: achievementTracking.mindfulBreaks });
    }
}

// Update days without all-in (call this daily)
function updateDaysWithoutAllIn() {
    if (!achievementTracking.lastAllInDate) {
        // If no all-in recorded, count from when tracking started
        achievementTracking.daysWithoutAllIn++;
    } else {
        const now = new Date();
        const lastAllIn = new Date(achievementTracking.lastAllInDate);
        const diffTime = Math.abs(now - lastAllIn);
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        achievementTracking.daysWithoutAllIn = diffDays;
    }
    
    window.daysWithoutAllIn = achievementTracking.daysWithoutAllIn;
    
    console.log(`[ACHIEVEMENT] Days without all-in: ${achievementTracking.daysWithoutAllIn}`);
    
    saveAchievementTracking();
    
    // Trigger achievement check
    if (typeof trackGameEvent === 'function') {
        trackGameEvent('days-without-allin', { days: achievementTracking.daysWithoutAllIn });
    }
}

// Daily update function for recovery path tracking
function updateRecoveryPathTracking() {
    const userId = getUserIdFromURL();
    if (!userId) return;
    
    // Check if we need to update days without all-in
    const lastUpdate = localStorage.getItem(`lastRecoveryUpdate_${userId}`);
    const now = new Date();
    const today = now.toDateString();
    
    if (lastUpdate !== today) {
        updateDaysWithoutAllIn();
        localStorage.setItem(`lastRecoveryUpdate_${userId}`, today);
        console.log('[ACHIEVEMENT] Daily recovery path tracking updated');
    }
}

// Add mindful break button functionality
function addMindfulBreakButton() {
    // Check if button already exists
    if (document.getElementById('mindful-break-btn')) return;
    
    // Create mindful break button
    const mindfulBreakBtn = document.createElement('button');
    mindfulBreakBtn.id = 'mindful-break-btn';
    mindfulBreakBtn.innerHTML = '🧘 Take Break';
    mindfulBreakBtn.className = 'mindful-break-button';
    mindfulBreakBtn.style.cssText = `
        position: fixed;
        top: 20px;
        right: 20px;
    ...
    mindfulBreakBtn.addEventListener('click', function() {
        if (typeof trackMindfulBreak === 'function') {
            trackMindfulBreak();
        }
        
        // Show feedback
        this.innerHTML = '✅ Break Taken!';
        this.style.background = 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)';
        
        setTimeout(() => {
            this.innerHTML = '🧘 Take Break';
            this.style.background = 'linear-gradient(135deg, #4ade80 0%, #22c55e 100%)';
        }, 2000);
        
        // Show educational message
        showEducationalTip('Great job taking a mindful break! This helps maintain healthy gaming habits.', 4000);
    });
    
    document.body.appendChild(mindfulBreakBtn);
}

// Track river card
function trackRiverCard() {
    // This will be called when the river card is dealt
    // We'll track if the user wins with this river card
    achievementTracking.wonWithRiver = false;
}

// Check if user won with river card
function checkRiverWin() {
    if (achievementTracking.wonWithRiver) {
        return true;
    }
    return false;
}
...
function getAchievementProgress() {
    return {
        ...
        'small-bet': achievementTracking.maxBetInHand >= 25,
        'medium-bet': achievementTracking.maxBetInHand >= 300,
        'high-roller': achievementTracking.maxBetInHand >= 800,
        'millionaire': achievementTracking.maxBetInHand >= 2000,
        'whale': achievementTracking.maxBetInHand >= 5000,
        
        // Winning achievements
        'first-win': achievementTracking.totalWins >= 1,
        'winning-streak': achievementTracking.consecutiveWins >= 4,
        'winning-master': achievementTracking.totalWins >= 25,
        'legend': achievementTracking.totalWins >= 75,
        'ultimate-streak': achievementTracking.consecutiveWins >= 8,
        
        // Experience achievements
        'first-game': achievementTracking.totalHandsPlayed >= 1,
        'experienced': achievementTracking.totalHandsPlayed >= 50,
        'veteran': achievementTracking.totalHandsPlayed >= 150,
        'champion': achievementTracking.totalHandsPlayed >= 50 && 
                 (achievementTracking.totalWins / achievementTracking.totalHandsPlayed) >= 0.75,
        'marathoner': achievementTracking.totalHandsPlayed >= 500,
        
        // Special achievements
        'river-king': achievementTracking.riverWins >= 1,
        'fold-master': achievementTracking.foldStreak >= 10,
        'collector': false, // Will be calculated by main app
        'all-rounder': Object.keys(achievementTracking.handTypesWon).length >= 10,
        'perfectionist': false, // Will be calculated by main app
        
        // RECOVERY PATH achievements
        'first-journal': false, // Will be set by main app
        'fold-discipline': achievementTracking.foldDisciplineStreak >= 3,
        'week-without-allin': achievementTracking.daysWithoutAllIn >= 7,
        'mindful-player': achievementTracking.mindfulBreaks >= 10,
        'recovery-champion': false // Will be calculated by main app
    };
}
...
function showAchievementNotification(achievementId, achievementName, description) {
    ...
    notification.innerHTML = `
        <div class="achievement-notification-content">
            ...
            <div class="achievement-notification-text">
                ...
                <div class="achievement-notification-desc">${description}</div>
            </div>
        </div>
    `;
...
function checkAchievements() {
    ...
    const achievements = {
        'small-bet': { name: 'Small Bet', description: 'Bet 25+ chips in one hand' },
        'medium-bet': { name: 'Medium Bet', description: 'Bet 300+ chips in one hand' },
        'high-roller': { name: 'High Roller', description: 'Bet 800+ chips in one hand' },
        'millionaire': { name: 'Millionaire', description: 'Bet 2000+ chips in one hand' },
        'whale': { name: 'Whale', description: 'Bet 5000+ chips in one hand' },
        'first-win': { name: 'First Win', description: 'Win your first hand' },
        'winning-streak': { name: 'Hot Streak', description: 'Win 4 hands in a row' },
        'winning-master': { name: 'Winning Master', description: 'Win 25+ hands' },
        'legend': { name: 'Legend', description: 'Win 75+ hands' },
        'ultimate-streak': { name: 'Ultimate Streak', description: 'Win 8 hands in a row' },
        'first-game': { name: 'First Game', description: 'Play your first hand' },
        'experienced': { name: 'Experienced', description: 'Play 50+ hands' },
        'veteran': { name: 'Veteran', description: 'Play 150+ hands' },
        'champion': { name: 'Champion', description: '75%+ win rate with 50+ hands' },
        'marathoner': { name: 'Marathoner', description: 'Play 500+ hands' },
        'river-king': { name: 'River King', description: 'Win with a river card' },
        'fold-master': { name: 'Fold Master', description: 'Fold 10 hands in a row' },
        'all-rounder': { name: 'All-Rounder', description: 'Win with every hand type' },
        'first-journal': { name: 'First Journal', description: 'Complete your first journal entry' },
        'fold-discipline': { name: 'Fold Discipline', description: 'Fold 3 big hands in a row' },
        'week-without-allin': { name: 'Week Without All-In', description: 'Go a week without going all-in' },
        'mindful-player': { name: 'Mindful Player', description: 'Take 10 mindful breaks' },
        'recovery-champion': { name: 'Recovery Champion', description: 'Complete the recovery path' }
    };
    
    // Check each achievement
    Object.keys(progress).forEach(achievementId => {
        if (progress[achievementId] && !unlockedAchievements[achievementId]) {
            // Achievement just unlocked!
            unlockedAchievements[achievementId] = { unlocked: true, unlockedAt: Date.now() };
            
            // Show notification
            const achievement = achievements[achievementId];
            if (achievement) {
                showAchievementNotification(achievementId, achievement.name, achievement.description);
    ...
    localStorage.setItem(`achievements_${userId}`, JSON.stringify(unlockedAchievements));
    
    // Update achievement display on the achievements page
    if (typeof updateAchievementDisplay === 'function') {
        updateAchievementDisplay();
    }
    
    // Also update the main app's achievement system if available
    if (typeof window.updateAchievementDisplay === 'function') {
        window.updateAchievementDisplay();
    }
}

// Helper function to extract hand type from reason
function getHandTypeFromReason(reason) {
    ...
    for (const handType of handTypes) {
        if (reason.toLowerCase().includes(handType.toLowerCase())) {
            return handType;
        }
    }
...
// Initialize achievement tracking when the game loads
document.addEventListener('DOMContentLoaded', function() {
    initializeAchievementTracking();
    console.log('[ACHIEVEMENT] Achievement tracking system loaded');
});

// Make functions available globally
window.achievementTracking = achievementTracking;
window.trackBetting = trackBetting;
window.trackHandCompletion = trackHandCompletion;
window.trackFold = trackFold;
window.trackRiverCard = trackRiverCard;
window.getAchievementProgress = getAchievementProgress;
window.saveAchievementTracking = saveAchievementTracking;
window.checkAchievements = checkAchievements;
window.showAchievementNotification = showAchievementNotification;

// RECOVERY PATH TRACKING - Make functions available globally
window.trackFoldDiscipline = trackFoldDiscipline;
window.trackAllIn = trackAllIn;
window.trackMindfulBreak = trackMindfulBreak;
window.updateDaysWithoutAllIn = updateDaysWithoutAllIn;
window.updateRecoveryPathTracking = updateRecoveryPathTracking;
window.addMindfulBreakButton = addMindfulBreakButton;

// Test function to verify achievement tracking
window.testAchievementTracking = function() {
    console.log('[ACHIEVEMENT] Testing achievement tracking...');
...
console.log('[ACHIEVEMENT] Achievement system fully loaded and ready');
...
let dailyPlayTime = 0;
let dailyHandsPlayed = 0;
let lastPlayDate = null;
let sessionLoss = 0; // Track session losses
let dailyLoss = 0; // Track daily losses
let startingChips = 0; // Track starting chips for session
let startingChipsToday = 0; // Track starting chips for today

// Responsible gaming settings
let responsibleGamingSettings = {
  sessionTimeLimit: 60, // minutes
  breakReminderInterval: 15, // minutes
...
/**
 * Initialize responsible gaming features
 */
function initializeResponsibleGaming() {
  console.log("[RG] Initializing responsible gaming features");
  
  // Load settings from localStorage
  loadResponsibleGamingSettings();
  ...
  // RECOVERY PATH TRACKING - Initialize recovery path features
  updateRecoveryPathTracking();
  // addMindfulBreakButton(); // Removed to disable the Take Break button
  
  console.log("[RG] Responsible gaming features initialized");
}

/**
 * Load responsible gaming settings from localStorage
 */
function loadResponsibleGamingSettings() {
  try {
    const saved = localStorage.getItem('responsibleGamingSettings');
    if (saved) {
      const parsed = JSON.parse(saved);
      responsibleGamingSettings = { ...responsibleGamingSettings, ...parsed };
    }
  } catch (error) {
    console.error("[RG] Error loading settings:", error);
  }
}

/**
 * Save responsible gaming settings to localStorage
 */
function saveResponsibleGamingSettings() {
  try {
    localStorage.setItem('responsibleGamingSettings', JSON.stringify(responsibleGamingSettings));
    console.log("[RG] Settings saved successfully");
  } catch (error) {
    console.error("[RG] Error saving settings:", error);
  }
}
...
function loadDailyStats() {
  try {
    ...
    if (saved) {
      ...
      if (stats.date === today) {
        dailyPlayTime = stats.playTime || 0;
        dailyHandsPlayed = stats.handsPlayed || 0;
    ...
    // Initialize starting chips for today - use current player chips or default
    const currentChips = typeof players[3] === 'number' ? players[3] : 1000;
    startingChipsToday = currentChips;
  } catch (error) {
    console.error("[RG] Error loading daily stats:", error);
  }
}
...
function saveDailyStats() {
  try {
    const stats = {
      ...
      handsPlayed: dailyHandsPlayed,
      dailyLoss: dailyLoss
    };
    localStorage.setItem('dailyStats', JSON.stringify(stats));
  } catch (error) {
    console.error("[RG] Error saving daily stats:", error);
  }
}

/**
 * Initialize loss tracking for new session
 */
function initializeLossTracking() {
  if (typeof startingChips !== 'number' || isNaN(startingChips)) {
    const currentChips = typeof players[3] === 'number' ? players[3] : 1000;
    startingChips = currentChips;
    console.log('[RG] Loss tracking initialized. Starting chips:', startingChips, 'Current chips:', currentChips);
  } else {
    console.log('[RG] Loss tracking already initialized. Starting chips:', startingChips);
  }
  sessionLoss = 0;
}
 ...
 */
function updateLossTracking() {
  const currentChips = typeof players[3] === 'number' ? players[3] : 0;
  const sessionStart = typeof startingChips === 'number' ? startingChips : 0;
  const dailyStart = typeof startingChipsToday === 'number' ? startingChipsToday : 0;
  // Only count loss if currentChips < sessionStart
  const sessionLossAmount = Math.max(0, sessionStart - currentChips);
  const dailyLossAmount = Math.max(0, dailyStart - currentChips);
  sessionLoss = sessionLossAmount;
  dailyLoss = dailyLossAmount;
  console.log('[RG] Loss tracking updated - Current chips:', currentChips, 'Session loss:', sessionLoss, 'Daily loss:', dailyLoss);
  console.log('[RG] Starting values - Session start:', sessionStart, 'Daily start:', dailyStart);
  checkLossLimits();
}

/**
 * Check loss limits and show warnings
 */
function checkLossLimits() {
  // Check session loss limit
  if (sessionLoss >= responsibleGamingSettings.sessionLossLimit) {
    showSessionLossLimitWarning();
    enforceSessionLossLimit();
  }
  
  // Check daily loss limit
  if (dailyLoss >= responsibleGamingSettings.dailyLossLimit) {
    showDailyLossLimitWarning();
    enforceDailyLossLimit();
  }
}

/**
 * Enforce session loss limit by disabling gameplay
 */
function enforceSessionLossLimit() {
  console.log("[RG] Enforcing session loss limit");
  ...
  enforcement.id = 'session-loss-enforcement';
  enforcement.style.cssText = `
    position: fixed;
    top: 20px;
    left: 50%;
    transform: translateX(-50%);
  ...
  enforcement.innerHTML = `
    ...
    <span style="margin-left: 8px;">Session Loss Limit Reached - Gameplay Disabled</span>
    <button onclick="endSession()" 
            style="background: #e74c3c; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; margin-left: 10px; font-size: 12px;">
      End Session
    </button>
  `;
  ...
  overlay.id = 'session-loss-overlay';
...
/**
 * Enforce daily loss limit by ending session
 */
function enforceDailyLossLimit() {
  console.log("[RG] Enforcing daily loss limit");
  ...
  // Show enforcement message
  const enforcement = document.createElement('div');
  enforcement.id = 'daily-loss-enforcement';
  enforcement.style.cssText = `
    position: fixed;
    top: 20px;
    left: 50%;
  ...
  enforcement.innerHTML = `
    <span style="font-size: 18px;">🚫</span>
    <span style="margin-left: 8px;">Daily Loss Limit Reached - Session Ending</span>
  ...
  // Auto-end session after 10 seconds
  setTimeout(() => {
    forceEndSession();
  }, 10000);
}
...
/**
 * Force end session (used when limits are reached)
 */
function forceEndSession() {
  console.log("[RG] Force ending session due to loss limits");
  
  // Remove enforcement messages
  const sessionEnforcement = document.getElementById('session-loss-enforcement');
  const dailyEnforcement = document.getElementById('daily-loss-enforcement');
  const sessionOverlay = document.getElementById('session-loss-overlay');
  const dailyOverlay = document.getElementById('daily-loss-overlay');
  
  if (sessionEnforcement) sessionEnforcement.remove();
  if (dailyEnforcement) dailyEnforcement.remove();
  if (sessionOverlay) sessionOverlay.remove();
  if (dailyOverlay) dailyOverlay.remove();
  
  // End the session
  endSession();
  
  // Show final warning
  showEducationalTip('Session ended due to loss limits. Please take a break and return later.', 8000);
}

/**
 * Remove loss limit enforcement (if user wants to continue)
 */
function removeLossLimitEnforcement() {
  console.log("[RG] Removing loss limit enforcement");
...
/**
 * Show session loss limit warning
 */
function showSessionLossLimitWarning() {
  ...
  warning.innerHTML = `
    ...
    <h3 style="color: #ffd700; margin: 10px 0;">Session Loss Limit Reached</h3>
    <p>You have reached your session loss limit of ${responsibleGamingSettings.sessionLossLimit} chips.</p>
    <p>Gameplay has been disabled. You can either continue playing or end your session.</p>
    <div style="margin-top: 15px;">
      <button onclick="removeLossLimitEnforcement(); this.parentElement.parentElement.remove()" 
              style="background: rgba(255,255,255,0.2); color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; margin: 0 5px;">
        Continue Playing
      </button>
      <button onclick="forceEndSession()" 
              style="background: #e74c3c; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; margin: 0 5px;">
        End Session
      </button>
    </div>
  `;
...
/**
 * Show daily loss limit warning
 */
function showDailyLossLimitWarning() {
  const warning = document.createElement('div');
  warning.style.cssText = `
    position: fixed;
    top: 50%;
  ...
  warning.innerHTML = `
    ...
    <h3 style="color: #ffd700; margin: 10px 0;">Daily Loss Limit Reached</h3>
    <p>You have reached your daily loss limit of ${responsibleGamingSettings.dailyLossLimit} chips.</p>
    <p>Please take a break and return tomorrow.</p>
    <div style="margin-top: 15px;">
      <button onclick="endSession()" 
...
function startSessionTimer() {
  if (!sessionStartTime) {
    ...
    console.log("[RG] Session started at:", new Date(sessionStartTime).toLocaleTimeString());
  }
...
function updateSessionTimer() {
  ...
  if (timerDisplay) {
    ...
    const sessionMinutes = elapsed / 60;
    timerDisplay.className = 'timer-text';
    
    if (sessionMinutes >= responsibleGamingSettings.sessionTimeLimit * 0.8) {
      timerDisplay.classList.add('warning');
    }
    if (sessionMinutes >= responsibleGamingSettings.sessionTimeLimit) {
 ...
 */
function updateDailyPlayTime() {
  if (sessionStartTime) {
    const sessionMinutes = Math.floor((Date.now() - sessionStartTime) / 60000);
    dailyPlayTime = sessionMinutes;
    
    // Check daily time limit
    if (dailyPlayTime >= responsibleGamingSettings.dailyTimeLimit) {
      showDailyLimitWarning('time');
    }
  }
}
...
function startBreakReminderTimer() {
  const interval = responsibleGamingSettings.breakReminderInterval * 60 * 1000; // Convert to milliseconds
...
function createSessionTimerDisplay() {
  ...
  const existingTimer = document.querySelector('.session-timer');
  if (existingTimer) {
  ...
  timerContainer.innerHTML = `
    <div class="timer-content">
      ...
      <span id="session-timer-display" class="timer-text">00:00:00</span>
    </div>
  `;
  ...
  reminder.innerHTML = `
    <div class="reminder-content">
      <div style="text-align: center; margin-bottom: 15px;">
        <span style="font-size: 24px;">⏰</span>
        <h3 style="color: white; margin: 10px 0;">Time for a Break!</h3>
      </div>
      <p style="color: white; text-align: center; margin-bottom: 20px; line-height: 1.4;">
        You've been playing for a while. Consider taking a short break to stretch, 
        get some water, or step away from the screen.
      </p>
      <div style="display: flex; gap: 10px; justify-content: center;">
        <button onclick="this.parentElement.parentElement.parentElement.remove()" 
                style="background: rgba(255,255,255,0.2); color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
          Take a Break
        </button>
        <button onclick="this.parentElement.parentElement.parentElement.remove()" 
                style="background: rgba(255,255,255,0.2); color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
...
function showSessionLimitWarning() {
  const warning = document.createElement('div');
  warning.className = 'session-limit-warning';
  warning.style.cssText = `
    position: fixed;
    top: 50%;
    left: 50%;
  ...
  warning.innerHTML = `
    <div style="text-align: center;">
      <span style="font-size: 24px;">⚠️</span>
      <h3 style="color: white; margin: 10px 0;">Session Time Limit Reached</h3>
      <p style="color: white; margin-bottom: 20px; line-height: 1.4;">
        You've reached your session time limit. Consider taking a longer break 
        or ending your session for today.
      </p>
      <div style="display: flex; gap: 10px; justify-content: center;">
        <button onclick="endSession()" 
                style="background: rgba(255,255,255,0.2); color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
        ...
        <button onclick="sessionStartTime = Date.now(); this.parentElement.parentElement.parentElement.remove(); console.log('[RG] Session continued after time limit at:', new Date(sessionStartTime).toLocaleTimeString());" 
                style="background: rgba(255,255,255,0.2); color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
          Continue
        </button>
      </div>
 ...
 */
function showDailyLimitWarning(type) {
  const warning = document.createElement('div');
  warning.className = 'daily-limit-warning';
  warning.style.cssText = `
    position: fixed;
    top: 50%;
    left: 50%;
  ...
  const message = type === 'time' 
    ? `You've reached your daily time limit of ${responsibleGamingSettings.dailyTimeLimit} minutes.`
    : `You've reached your daily hands limit of ${responsibleGamingSettings.dailyHandsLimit} hands.`;
  
  warning.innerHTML = `
    <div style="text-align: center;">
      <span style="font-size: 24px;">🚫</span>
      <h3 style="color: white; margin: 10px 0;">Daily Limit Reached</h3>
      <p style="color: white; margin-bottom: 20px; line-height: 1.4;">
        ${message} Please consider taking a break and returning tomorrow.
      </p>
      <div style="display: flex; gap: 10px; justify-content: center;">
        <button onclick="endSession()" 
                style="background: rgba(255,255,255,0.2); color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
...
function endSession() {
  ...
  // --- NEW: Mark session ended for today ---
  try {
    const userId = typeof getUserIdFromURL === 'function' ? getUserIdFromURL() : null;
    if (userId) {
      const today = new Date().toISOString().split('T')[0];
      localStorage.setItem(`sessionEnded_${userId}`, today);
    }
  } catch (e) { console.warn('Failed to set session ended flag:', e); }
  // --- END NEW ---
...
function showSessionSummary() {
  ...
  const sessionMinutes = Math.floor((Date.now() - (sessionStartTime || Date.now())) / 60000);
  
  summary.innerHTML = `
    <div style="text-align: center;">
      <span style="font-size: 24px;">📊</span>
      <h3 style="color: #ffd700; margin: 10px 0;">Session Summary</h3>
      <div style="color: white; text-align: left; margin: 20px 0;">
        <p><strong>Session Duration:</strong> ${sessionMinutes} minutes</p>
        <p><strong>Daily Loss:</strong> ${dailyLoss} chips (Limit: ${responsibleGamingSettings.dailyLossLimit})</p>
        <p><strong>Hands Played Today:</strong> ${dailyHandsPlayed}</p>
        <p><strong>Total Time Today:</strong> ${dailyPlayTime} minutes</p>
        <p><strong>Daily Time Limit:</strong> ${responsibleGamingSettings.dailyTimeLimit} minutes</p>
        <p><strong>Daily Hands Limit:</strong> ${responsibleGamingSettings.dailyHandsLimit}</p>
      </div>
      <p style="color: #bdc3c7; font-size: 14px;">
        Thank you for playing responsibly! You'll be redirected to the main menu in 5 seconds.
      </p>
    </div>
  `;
...
function checkDailyLimits() {
  ...
  // Check hands limit
  if (dailyHandsPlayed >= responsibleGamingSettings.dailyHandsLimit) {
    showDailyLimitWarning('hands');
  }
}

/**
 * Track hand completion for daily limits
 */
function trackHandForDailyLimits() {
  dailyHandsPlayed++;
  saveDailyStats();
...
/**
 * Show responsible gaming settings modal
 */
function showResponsibleGamingSettings() {
  const modal = document.getElementById('responsible-gaming-settings-modal');
  if (!modal) {
    console.error("[RG] Responsible gaming modal not found");
    return;
  }
  
  // Populate current settings
  document.getElementById('session-time-limit').value = responsibleGamingSettings.sessionTimeLimit;
  document.getElementById('break-reminder').value = responsibleGamingSettings.breakReminderInterval;
  document.getElementById('session-loss-limit').value = responsibleGamingSettings.sessionLossLimit;
  document.getElementById('daily-loss-limit').value = responsibleGamingSettings.dailyLossLimit;
  document.getElementById('show-educational-tips').checked = responsibleGamingSettings.showEducationalTips;
  document.getElementById('show-probabilities').checked = responsibleGamingSettings.showProbabilities;
  
  // Add daily limits section if it doesn't exist
  let dailyLimitsSection = modal.querySelector('.daily-limits-section');
  if (!dailyLimitsSection) {
    dailyLimitsSection = document.createElement('div');
    dailyLimitsSection.className = 'settings-section daily-limits-section';
    dailyLimitsSection.innerHTML = `
      <h4>📅 Daily Limits</h4>
      <div class="setting-item">
        <label for="daily-time-limit">Daily time limit (minutes):</label>
        <input type="number" id="daily-time-limit" min="30" max="480" value="${responsibleGamingSettings.dailyTimeLimit}">
      </div>
      <div class="setting-item">
        <label for="daily-hands-limit">Daily hands limit:</label>
        <input type="number" id="daily-hands-limit" min="10" max="200" value="${responsibleGamingSettings.dailyHandsLimit}">
      </div>
    `;
    
    // Insert before the last settings section
    ...
    lastSection.parentNode.insertBefore(dailyLimitsSection, lastSection.nextSibling);
  }
  
  // Add help resources section if it doesn't exist
  let helpSection = modal.querySelector('.help-resources');
  if (!helpSection) {
    helpSection = document.createElement('div');
    helpSection.className = 'help-resources';
    helpSection.innerHTML = `
      <p>Need Help?</p>
      <a href="https://www.gamblersanonymous.org/" target="_blank">Gamblers Anonymous</a>
      <a href="https://www.ncpgambling.org/" target="_blank">National Council on Problem Gambling</a>
      <a href="https://www.begambleaware.org/" target="_blank">BeGambleAware</a>
      <a href="tel:1-800-522-4700" target="_blank">National Problem Gambling Helpline: 1-800-522-4700</a>
      <div style="margin-top: 10px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.1);">
        <p style="margin-bottom: 6px; color: #4ecdc4; font-size: 11px;">Test Educational Features:</p>
        <button onclick="showEducationalTip('This is a test educational tip! Educational features are working correctly.', 5000)" 
                style="background: rgba(76, 205, 196, 0.2); color: #4ecdc4; border: 1px solid #4ecdc4; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 10px; margin: 2px;">
          Test Tip
        </button>
        <button onclick="showGameplayEducationalTip()" 
                style="background: rgba(76, 205, 196, 0.2); color: #4ecdc4; border: 1px solid #4ecdc4; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 10px; margin: 2px;">
          Gameplay Tip
        </button>
        <button onclick="showProbabilityTip()" 
                style="background: rgba(76, 205, 196, 0.2); color: #4ecdc4; border: 1px solid #4ecdc4; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 10px; margin: 2px;">
          Probability Tip
        </button>
        <div style="margin-top: 8px; padding-top: 6px; border-top: 1px solid rgba(255,255,255,0.1);">
          <p style="margin-bottom: 6px; color: #e74c3c; font-size: 11px;">Test Loss Limits:</p>
          <button onclick="showSessionLossLimitWarning()" 
                  style="background: rgba(231, 76, 60, 0.2); color: #e74c3c; border: 1px solid #e74c3c; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 10px; margin: 2px;">
            Session Loss Warning
          </button>
          <button onclick="showDailyLossLimitWarning()" 
                  style="background: rgba(231, 76, 60, 0.2); color: #e74c3c; border: 1px solid #e74c3c; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 10px; margin: 2px;">
            Daily Loss Warning
          </button>
        </div>
...
function saveResponsibleGamingSettings() {
  ...
  responsibleGamingSettings.sessionTimeLimit = parseInt(document.getElementById('session-time-limit').value) || 60;
  responsibleGamingSettings.breakReminderInterval = parseInt(document.getElementById('break-reminder').value) || 15;
  responsibleGamingSettings.sessionLossLimit = parseInt(document.getElementById('session-loss-limit').value) || 500;
  responsibleGamingSettings.dailyLossLimit = parseInt(document.getElementById('daily-loss-limit').value) || 2000;
  responsibleGamingSettings.dailyTimeLimit = parseInt(document.getElementById('daily-time-limit')?.value) || 120;
  responsibleGamingSettings.dailyHandsLimit = parseInt(document.getElementById('daily-hands-limit')?.value) || 50;
  responsibleGamingSettings.showEducationalTips = document.getElementById('show-educational-tips').checked;
  responsibleGamingSettings.showProbabilities = document.getElementById('show-probabilities').checked;
  
  // Save to localStorage
  saveResponsibleGamingSettings();
  
  // Restart break reminder timer with new interval
  if (breakReminderTimer) {
    clearInterval(breakReminderTimer);
    startBreakReminderTimer();
  }
  ...
  // Close modal
  closeResponsibleGamingSettings();
  
  // Show confirmation
  showEducationalTip('Settings saved successfully! Your responsible gaming preferences have been updated.');
}

/**
 * Close responsible gaming settings modal
 */
function closeResponsibleGamingSettings() {
  const modal = document.getElementById('responsible-gaming-settings-modal');
  if (modal) {
 ...
 */
function showEducationalTip(message, duration = 5000) {
  if (!responsibleGamingSettings.showEducationalTips) return;
  
  // Create or get the tips container
  ...
  if (!tipsContainer) {
    ...
    tipsContainer.style.top = '20px';
    tipsContainer.style.right = '20px';
    tipsContainer.style.display = 'flex';
    tipsContainer.style.flexDirection = 'column';
    tipsContainer.style.gap = '12px';
    tipsContainer.style.zIndex = '9999';
    document.body.appendChild(tipsContainer);
  }
  const tip = document.createElement('div');
  tip.className = 'educational-tip';
  tip.innerHTML = `
  ...
  `;
  tip.style.position = 'static'; // So it stacks in the container
...
/**
 * Show educational tip during gameplay
 */
function showGameplayEducationalTip() {
  if (!responsibleGamingSettings.showEducationalTips) return;
  // Don't show tips too frequently
  const lastTipTime = localStorage.getItem('lastEducationalTipTime');
  const now = Date.now();
  if (lastTipTime && (now - parseInt(lastTipTime)) < 60000) return; // 60 second cooldown
  // Only show tip 10% of the time
  if (Math.random() > 0.10) return;
  const tips = [
    "💡 Remember: It's okay to fold! Don't feel pressured to play every hand.",
    "💡 Take breaks regularly to stay focused and make better decisions.",
    "💡 Set limits before you start playing to help manage your time and chips.",
    "💡 Watch your opponents' betting patterns - they can tell you about their hand strength.",
    "💡 Position matters! You have more information when you act later in the betting round.",
    "💡 Don't chase losses. If you're having a bad run, consider taking a break.",
    "💡 Practice good bankroll management - never bet more than you can afford to lose.",
    "💡 Pay attention to pot odds when deciding whether to call or fold.",
    "💡 Remember that poker is a game of skill and patience, not just luck.",
    "💡 Keep track of your session time to maintain healthy gaming habits.",
    "💡 If you're feeling frustrated or emotional, it's a good time to step away.",
    "💡 Focus on making good decisions rather than just winning or losing.",
    "💡 Learn from each hand - even losses can teach you valuable lessons.",
    "💡 Stay hydrated and take eye breaks during long sessions.",
    "💡 Don't play when you're tired - fatigue leads to poor decisions.",
    // New unique tips:
    "💡 Bluffing works best against fewer opponents.",
    "💡 Don't slow play your strong hands too often.",
    "💡 If you can't beat the hand, don't call the bet.",
    "💡 The best players fold more often than you think.",
    "💡 Don't get attached to a hand just because you started strong.",
    "💡 If the board is scary, it's probably scary for your opponent too.",
    "💡 Don't be afraid to re-raise with a strong hand.",
    "💡 If you don't know what to do, folding is usually safe.",
    "💡 Track your results to spot leaks in your game.",
    "💡 Don't let one bad beat ruin your session.",
    "💡 The best hand pre-flop doesn't always win at showdown.",
    "💡 If you never get caught bluffing, you aren't bluffing enough.",
    "💡 Don't limp in with weak hands from early position.",
    "💡 Use your position to control the size of the pot."
  ];
  const randomTip = tips[Math.floor(Math.random() * tips.length)];
  showEducationalTip(randomTip, 8000);
  localStorage.setItem('lastEducationalTipTime', now.toString());
}

/**
 * Show hand strength educational tip
 */
function showHandStrengthTip(handType, strength) {
  if (!responsibleGamingSettings.showEducationalTips) return;
  
  const handTips = {
    'High Card': "💡 High Card: The weakest hand. Consider folding unless you have a very strong high card like an Ace.",
    'Pair': "💡 Pair: A decent starting hand. Play cautiously and watch for better hands on the board.",
    'Two Pair': "💡 Two Pair: A strong hand! But be careful of straights and flushes on the board.",
    'Three of a Kind': "💡 Three of a Kind: Very strong! This hand wins most of the time.",
    'Straight': "💡 Straight: Excellent hand! Five consecutive cards of different suits.",
    'Flush': "💡 Flush: Great hand! Five cards of the same suit, but watch for higher flushes.",
    'Full House': "💡 Full House: Very strong! Three of a kind plus a pair.",
    'Four of a Kind': "💡 Four of a Kind: Extremely strong! Almost unbeatable.",
    'Straight Flush': "💡 Straight Flush: Incredible! Five consecutive cards of the same suit.",
    'Royal Flush': "💡 Royal Flush: The best possible hand! Ace-high straight flush."
  };
  
  const tip = handTips[handType] || `💡 ${handType}: ${strength}`;
  showEducationalTip(tip, 6000);
}

/**
 * Show betting educational tip
 */
function showBettingTip(betType, amount) {
  if (!responsibleGamingSettings.showEducationalTips) return;
  
  const bettingTips = {
    'small': "💡 Small Bet: Good for building the pot when you have a decent hand.",
    'medium': "💡 Medium Bet: Shows strength and can help you control the pot size.",
    'large': "💡 Large Bet: Indicates a very strong hand. Be confident in your cards!",
    'all-in': "💡 All-In: High risk, high reward. Make sure you have a strong hand or good odds.",
    'check': "💡 Check: Free to see the next card. Good when you're unsure about your hand.",
    'fold': "💡 Fold: Smart play when your hand is weak. Save your chips for better opportunities."
  };
  
  const tip = bettingTips[betType] || `💡 ${betType}: ${amount}`;
  showEducationalTip(tip, 4000);
}

/**
 * Show probability educational tip
 */
function showProbabilityTip() {
  if (!responsibleGamingSettings.showProbabilities) return;
  
  const probabilityTips = [
    "📊 Probability Tip: You have about a 1 in 2.5 chance of getting a pair in your starting hand.",
    "📊 Probability Tip: The odds of getting a flush draw on the flop are about 1 in 3.",
    "📊 Probability Tip: You have about a 1 in 8 chance of making a straight with suited connectors.",
    "📊 Probability Tip: The probability of getting dealt pocket aces is about 1 in 221.",
    "📊 Probability Tip: You have about a 1 in 4 chance of improving your hand on the turn.",
    "📊 Probability Tip: The odds of making a full house with a pair on the flop are about 1 in 12."
  ];
  
  const randomTip = probabilityTips[Math.floor(Math.random() * probabilityTips.length)];
  showEducationalTip(randomTip, 7000);
}

/**
 * Show session timer with current stats
 */
function showSessionStats() {
  ...
  stats.innerHTML = `
    <div style="text-align: center;">
      <span style="font-size: 24px;">📈</span>
      <h3 style="color: #ffd700; margin: 10px 0;">Current Session Stats</h3>
      <div style="color: white; text-align: left; margin: 20px 0;">
        <p><strong>Session Duration:</strong> ${sessionMinutes} minutes</p>
        <p><strong>Daily Loss:</strong> ${dailyLoss} chips (Limit: ${responsibleGamingSettings.dailyLossLimit})</p>
        <p><strong>Hands Played Today:</strong> ${dailyHandsPlayed}</p>
...
// Hook into existing nextRound function to track hands
const originalNextRound = nextRound;
nextRound = function() {
  // Call original function
  originalNextRound.apply(this, arguments);
  
  // Track hand for daily limits
  trackHandForDailyLimits();
  
  // Show educational tip occasionally during gameplay
  if (Math.random() < 0.3) { // 30% chance
    setTimeout(() => {
      showGameplayEducationalTip();
    }, 2000);
  }
  
  // Show probability tip occasionally
...
// Hook into player actions to show betting tips
const originalPlayerAction = playerAction;
playerAction = function(action) {
  // Call original function
  originalPlayerAction.apply(this, arguments);
  
  // Show betting educational tip
  setTimeout(() => {
    showBettingTip(action, '');
  }, 1000);
};

// Hook into showdown to show hand strength tips
const originalShowdown = showdown;
showdown = function() {
  // Call original function
  originalShowdown.apply(this, arguments);
  
  // Show hand strength tip for the winning hand
  setTimeout(() => {
    // Get the winning hand type from the game state
    const playerCards = players[3]; // Human player
    const allCards = [...playerCards, ...communityCards];
    const handAnalysis = analyzeHand(allCards);
    if (handAnalysis && handAnalysis.handType) {
      showHandStrengthTip(handAnalysis.handType, handAnalysis.description);
    }
  }, 3000);
};

// Hook into awardPotToPlayer to show tips for wins/losses and track losses
const originalAwardPotToPlayer = awardPotToPlayer;
awardPotToPlayer = function(playerIdx, reason) {
  // Call original function
  originalAwardPotToPlayer.apply(this, arguments);
  
  // Update loss tracking after pot is awarded
  setTimeout(() => {
    updateLossTracking();
  }, 1000);
  
  // Show educational tip based on the result
  setTimeout(() => {
    if (playerIdx === 3) { // Human player won
      showEducationalTip("🎉 Great win! Remember to celebrate responsibly and consider taking a short break.", 6000);
    } else { // Human player lost
      showEducationalTip("💪 Don't worry about the loss! Focus on making good decisions rather than just winning.", 6000);
    }
  }, 2000);
};

// Add periodic educational tips during gameplay
function startEducationalTipTimer() {
  setInterval(() => {
    if (responsibleGamingSettings.showEducationalTips && Math.random() < 0.1) { // 10% chance every interval
      showGameplayEducationalTip();
    }
  }, 60000); // Check every minute
}

// Initialize educational tip timer when responsible gaming starts
const originalInitializeResponsibleGaming = initializeResponsibleGaming;
initializeResponsibleGaming = function() {
  // Call original function
  originalInitializeResponsibleGaming.apply(this, arguments);
  
  // Start educational tip timer
  startEducationalTipTimer();
  
  // Show welcome educational tip
  setTimeout(() => {
    showEducationalTip("🎮 Welcome! Remember to play responsibly and take breaks when needed.", 8000);
  }, 3000);
};

// Add keyboard shortcut for session stats (Ctrl+Shift+S)
document.addEventListener('keydown', function(e) {
  ...
  // Add keyboard shortcut for manual loss tracking update (Ctrl+Shift+L)
  if (e.ctrlKey && e.shiftKey && e.key === 'L') {
    e.preventDefault();
    updateLossTracking();
    showEducationalTip('Loss tracking updated manually. Session loss: ' + sessionLoss + ', Daily loss: ' + dailyLoss, 3000);
  }
});

// Initialize responsible gaming when the game starts
document.addEventListener('DOMContentLoaded', function() {
...
// Make responsible gaming functions available globally
window.showResponsibleGamingSettings = showResponsibleGamingSettings;
window.saveResponsibleGamingSettings = saveResponsibleGamingSettings;
window.closeResponsibleGamingSettings = closeResponsibleGamingSettings;
window.showSessionStats = showSessionStats;
window.endSession = endSession;
window.showEducationalTip = showEducationalTip;
window.showGameplayEducationalTip = showGameplayEducationalTip;
window.showHandStrengthTip = showHandStrengthTip;
window.showBettingTip = showBettingTip;
window.showProbabilityTip = showProbabilityTip;
window.updateLossTracking = updateLossTracking;
window.checkLossLimits = checkLossLimits;
window.enforceSessionLossLimit = enforceSessionLossLimit;
window.enforceDailyLossLimit = enforceDailyLossLimit;
window.forceEndSession = forceEndSession;
window.removeLossLimitEnforcement = removeLossLimitEnforcement;

/**
 * Reset starting chips for a new session (call this when starting a new game)
 */
function resetSessionStartingChips() {
  const currentChips = typeof players[3] === 'number' ? players[3] : 1000;
  startingChips = currentChips;
  sessionLoss = 0;
  console.log("[RG] Session starting chips reset to:", startingChips);
}
...
function resetLossTracking() {
  ...
  sessionLoss = 0;
  dailyLoss = 0;
  console.log("[RG] Loss tracking manually reset. Starting chips:", startingChips);
  showEducationalTip('Loss tracking has been reset. Starting chips: ' + startingChips, 3000);
}

/**
 * Continue playing after session time limit
 */
function continueAfterSessionLimit() {
  // Reset session start time to allow continued play
  sessionStartTime = Date.now();
  
  // Remove the warning modal - try multiple methods
  let warning = document.querySelector('.session-limit-warning');
  if (!warning) {
    // Try alternative selectors
    warning = document.querySelector('[class*="session-limit"]');
  }
  if (!warning) {
    // Try finding by content
    const allDivs = document.querySelectorAll('div');
    for (let div of allDivs) {
      if (div.textContent && div.textContent.includes('Session Time Limit Reached')) {
        warning = div;
        break;
      }
    }
  ...
  if (warning) {
    warning.remove();
    console.log("[RG] Session limit warning modal removed successfully");
  } else {
    console.log("[RG] Could not find session limit warning modal to remove");
  }
  
  console.log("[RG] Session continued after time limit at:", new Date(sessionStartTime).toLocaleTimeString());
}

// Add reset functions to global scope
window.resetSessionStartingChips = resetSessionStartingChips;
window.resetLossTracking = resetLossTracking;
window.continueAfterSessionLimit = continueAfterSessionLimit;

console.log('[RG] Responsible gaming system loaded and ready');